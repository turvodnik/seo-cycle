...
Скрипты и агент получают значения только в окружении дочернего процесса, не читая их сами. Команды, которые ходят к провайдерам (`pulse`, `doctor`, `sync`, `ads`, `feed`, `cohorts`, `notify`), лончер `seo-cycle` сам перезапускает под `ai-secret run <scope>`, если ключей pulse нет в окружении, а `ai-secret` найден; если его нет — печатает одну строку предупреждения в stderr и идёт дальше (прогон на старом срезе никогда не бывает тихим). Ручной эквивалент и способ для launchd/cron (обёртка зашивается в сам job):
