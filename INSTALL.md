...
`install.sh --project` ставит seo-cycle как **версионированное хранилище + project-local attach**: общий код в `~/.codex/vendor/`, в проекте — симлинк `.agents/external/seo-cycle` на конкретную версию и поверхности `./.codex/skills/seo-cycle`, `./.claude/skills/seo-cycle`. Если проект не подключали, seo-cycle в нём не появляется и не читается. Полный vendor clone в проект доступен через `--vendor-local`; legacy global skill exposure только через `--global-skill`. Attach создаёт `AGENTS.md` (+ симлинки `CLAUDE.md`/`GEMINI.md`), `.env.example` (только имена ключей — значения в Keychain через `ai-secret`), дефолтный `.gitignore` и запускает `init-project.sh`. WordPress/Novomira MCP не создаётся автоматически; включай его только флагом `--with-wordpress-mcp` или явной командой. Wizard задаёт 7 обязательных вопросов (имя, домен, тип проекта, CMS, язык/регион, governance/бюджет-профиль, monthly paid API budget) — по умолчанию не тратит токены и деньги без approval; LLM-бюджет, automation mode, картинки и подробный intake спрашиваются только при ответе «y» на завершающий вопрос «Уточнить картинки, расписания и подробный intake сейчас?» (T-107), иначе берутся дефолты из шаблона.
...
После setup/control-plane запусти Knowledge Hub: `bash ./.codex/skills/seo-cycle/scripts/knowledge/wiki-refresh-all.sh`. Он создаёт `seo/knowledge/wiki/` как источник правды по проекту: правила, статьи, категории, бренды, товары, internal links, API catalog, review/comparison candidates и `latest-context-pack.md`. Затем `bash ./.codex/skills/seo-cycle/scripts/knowledge/graphify-refresh.sh --live` (`--live` — согласие на расход токенов LLM CLI/API-ключа; без него при доступном LLM-пути — код 3, T-069) построит Graphify-граф через Antigravity/Gemini CLI/API или безопасно запишет degraded status, если Graphify ещё не установлен. Перед правкой страницы используй `wiki-preflight.py`; перед публикацией публичного текста — `content-taste-gate.py`. Подробно: `docs/knowledge-hub.md`.
...
Брокер берётся по каноническому пути `~/.local/bin/ai-secret`; `PATH` не используется (брокер получает значения по stdin, и каталог раньше в `PATH` не должен уметь его подменить). Установлен в другое место — задай абсолютный путь в переменной окружения `SEO_CYCLE_AI_SECRET` (только окружение процесса: из `.env` она не читается). Без брокера `auth login`/`auth set` завершаются с кодом 3 и сообщением «секреты не подключены» — тихой записи в `.env` нет. Тот же результат руками, по чек-листу из валидатора (человек вводит значение скрытым вводом, агент значений не видит):
...
переменная `SEO_CYCLE_REGISTRY`.

Автоматическая миграция легаси-файла (config/projects-registry.yaml внутри
clone/vendor-store — без обратных кавычек намеренно: этот путь больше не существует в
дереве по умолчанию, `tests/test_docs_links.py` не резолвит его) работает **только при свежем клоне или при первом обращении к
дереву, где легаси-файл ещё физически лежит на диске**. Она НЕ спасает тот же самый
переход, ради которого написана: если у вас уже есть работающий клон со старой версией
(файл ещё tracked) и вы обновляете его командой `git pull` до версии с этим фиксом, git
удаляет с диска файл, переставший быть tracked, — это происходит как часть самого
`pull`, до того как новый код на что-либо успевает среагировать. Источник миграции
исчезает раньше, чем миграция получает шанс его увидеть, и `init-project.sh` создаст на
его месте пустой реестр с нуля.

**Поэтому перед обновлением существующего клона командой `git pull` через эту версию —
скопируйте реестр вручную, до pull:**
```bash
mkdir -p ~/.seo-cycle
cp config/projects-registry.yaml ~/.seo-cycle/projects-registry.yaml   # выполнить ДО git pull
git pull
```
После этого автоматическая миграция просто не понадобится (целевой файл уже на месте,
резолвер его не тронет), а команда `seo-cycle run monthly --all` продолжит видеть весь
портфель. Миграция без ручного шага реально помогает только там, где легаси-файл
физически переживает обновление, а не удаляется им, — например, read-only
version-снапшот (T-049), который получает код этого фикса вместе с уже лежащим внутри
легаси-реестром одним и тем же checkout, а не через `git pull` поверх старого рабочего
дерева.
