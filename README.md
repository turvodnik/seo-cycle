...
| **Technical site tools** | `link-audit.py`, `redirect-map-audit.py`, `lighthouse-audit.py`, `serpstat-audit.py`, `labrika-source-pack.py` — broken links, redirect maps, Lighthouse/CWV, guarded Serpstat Site Audit API и Labrika manual/export ingestion. Live HTTP/API запускаются только явным `--live`; Serpstat требует `SERPSTAT_API_KEY` и credit/budget approval. |
