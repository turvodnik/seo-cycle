...
### Утро оператора: три команды

Ежедневный SEO-срез не требует знать все команды `seo-cycle` — хватает трёх:
...
- `pulse` — раз в день подтягивает свежие позиции и обновляет прогресс/алерты.
- `status` — быстрый снимок: свежесть данных, эскалации, следующая команда.
- `web` — то же самое, но глазами: визуальный агентский дашборд в браузере.

Остальное — по мере нужды; `seo-cycle --help` группирует все команды по смыслу (Сегодня/Контент/Деньги/Клиенты/Настройка/Прочее).
