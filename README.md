...
| **Staged orchestrator pilot** | `seo-cycle-run.py --goal "..." --write`, `--stage-template research-package --package seo/research-package --write` или `--stage-file stages.yaml --write` запускает декларативные стадии по схеме `stage -> gate -> repair -> rerun -> next stage`, по умолчанию до 5 repair-попыток, пишет `seo/orchestrator/latest-run.md/json`, stage reports и blocker reports. |
