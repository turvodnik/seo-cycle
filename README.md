...
**Версия 1.54.0** · универсальный SEO/контент-цикл-оркестратор для Codex CLI и Claude Code.
...
| **Research package quality + repair + deep briefs** | `research-package-quality.py <package> --write` фейлит site-level research package при пустой SERP-валидации, URL/cluster drift, грязном GSC, дублях briefs, orphan URLs, entity-map drift, неагрегированном Google NLP, неиспользованных AI Overview/GEO signals и слабом E-E-A-T/evidence layer. Repair layer закрывает findings отдельными артефактами: `semantic-core-clean.py`, `semantic-core-resync.py`, `entity-map-sync.py`, `google-nlp-aggregate.py`, `orphan-url-resolver.py`, `serp-validation-plan.py`, `spoke-opportunity-audit.py`, `entity-graph-quality.py`, `draft-quality-gate.py`. После этого `page-outline-v2.py <package> --all-mvp --write` или `--priority P1 --write` превращает правильную архитектуру в секционные H2/H3 брифы с copywriting playbook, writer prompt packet, source slots, Answer Units, schema, links, synthetic prompts и no-fabricated-E-E-A-T guard. |
