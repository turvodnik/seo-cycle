...
**Версия 1.55.0** · универсальный SEO/контент-цикл-оркестратор для Codex CLI и Claude Code.
...
| **Research package quality + repair + deep briefs** | `research-package-quality.py <package> --write` фейлит site-level research package при пустой SERP-валидации, URL/cluster drift, грязном GSC, дублях briefs, orphan URLs, entity-map drift, неагрегированном Google NLP, неиспользованных AI Overview/GEO signals и слабом E-E-A-T/evidence layer. `research-package-repair.py <package> --write` запускает весь repair layer: `semantic-core-clean.py`, `semantic-core-resync.py`, `entity-map-sync.py`, `google-nlp-aggregate.py`, `orphan-url-resolver.py`, `serp-validation-plan.py`, `spoke-opportunity-audit.py`, `entity-graph-quality.py`. После quality rerun `page-outline-v2.py <package> --all-mvp --write` или `--priority P1 --write` превращает правильную архитектуру в секционные H2/H3 брифы, `page-outline-quality.py` проверяет их, а `draft-quality-gate.py` проверяет готовый черновик. |
