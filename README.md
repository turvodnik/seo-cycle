...
Универсальный SEO/контент-цикл-оркестратор для Codex CLI и Claude Code. Текущая версия — файл [`VERSION`](VERSION) в корне репозитория (номер в тексте README быстро устаревает, поэтому здесь не дублируется).
