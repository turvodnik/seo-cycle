...
**Версия 1.63.0** · универсальный SEO/контент-цикл-оркестратор для Codex CLI и Claude Code.
...
# v1.63 orchestrator pilot: stage -> gate -> repair -> rerun
...
Staged orchestrator pilot — новый v1.63 слой управления стадиями: [docs/orchestrator.md](docs/orchestrator.md). Он не заменяет старые команды, а оборачивает их в проверяемый контракт: команда стадии, gate, repair-команды, лимит попыток и blocker report.
...
| **Staged orchestrator pilot** | `seo-cycle-run.py --goal "..." --write`, `--stage-template setup-readiness --goal "first SEO setup" --write`, `--stage-template research-package --package seo/research-package --write`, `--stage-template copywriting --draft <draft.md> --write` или `--stage-file seo/stages/*.yaml --write` запускает декларативные стадии по схеме `stage -> gate -> repair -> rerun -> next stage`, по умолчанию до 5 repair-попыток, пишет `seo/orchestrator/latest-run.md/json`, stage reports и blocker reports. `stage-template-export.py --write` создаёт project-local шаблоны `seo/stages/setup-readiness.yaml`, `research-package.yaml`, `copywriting-draft.yaml` без секретов и без перезаписи ручных правок. `orchestrator-panel.py` читает latest run как read-only panel без command logs/stdout/stderr и без запуска paid/browser/publish действий. |
