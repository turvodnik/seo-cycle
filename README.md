...
**Версия 1.46.0** · универсальный SEO/контент-цикл-оркестратор для Codex CLI и Claude Code.
...
| **Project-local MCP** | `project-mcp-config.py --write` creates `./.codex/config.toml` for optional WordPress/Novomira MCP without secrets. Each project owns its URL/user/password in local `.env`, so adding another project does not overwrite other WordPress MCP targets. |
