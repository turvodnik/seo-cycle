...
| **Масштаб** | `projects-registry.yaml` (локальный, из `.example`) + `init-project.sh` — новый проект одной командой. |
...
│   ├── projects-registry.example.yaml   # шаблон; реальный — projects-registry.yaml, в .gitignore
│   └── triggers.yaml
