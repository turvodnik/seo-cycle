...
- перед началом большого цикла обновлять compact readiness, setup blueprint, upgrade/access assistants и gap audit: `python3 ~/.codex/skills/seo-cycle/scripts/setup-control-plane.py --write`;
...
- перед чтением подробных setup-отчётов строить context pack, setup blueprint, upgrade/access assistants, gap audit/questionnaire и launch contract: `context-pack.py --write`, `setup-blueprint.py --write`, `project-upgrade-assistant.py --write`, `access-key-assistant.py --write`, `setup-gap-audit.py --write`, после заполнения questionnaire — `setup-answer-plan.py --write`, затем `launch-plan.py --write`; начинать с `seo/setup/context-pack.md`, потом `seo/setup/setup-blueprint.md`, `seo/setup/upgrade-questionnaire.csv`, `seo/setup/access-key-assistant.md`, потом `seo/setup/setup-questionnaire.csv` / `seo/setup/setup-gap-audit.md`, после заполнения CSV — `seo/setup/setup-answer-plan.md`, затем `seo/setup/launch-plan.md`;
