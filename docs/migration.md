...
## v1.x → v2.0 (модульная архитектура + версионированное хранилище)

Для проекта-потребителя v2 **не ломает ничего сам по себе** — переход происходит только при явном re-pin:

1. **Пины защищают.** Проект, запинованный на v1.x (`.agents/external-skills.lock.yaml`), продолжает работать со своим worktree/checkout. `install.sh --update` лишь скачивает новые теги в хранилище.
2. **Re-pin на v2:** `install.sh --project <dir> --pin v2.0.0 --sync`. Поверхности перегенерируются; появляется модульная раскладка `skills/*`.
3. **Пути совместимы.** Старые ссылки `templates/*.template.md` и `prompts/*.md` работают через симлинк-шимы (удаление шимов — в v3). `SKILL.md`/`AGENTS.md` остаются точками входа; монолитное содержимое фаз теперь в `skills/<модуль>/SKILL.md`.
4. **Что проверить после re-pin:** `seo-cycle doctor`; `python3 scripts/skill-manifest-validate.py`; проектные `delegate.*` — если делегат ссылался на текст фазы монолита, замени на модуль (`skills/seo-entity-map` и т.д.).
5. **Секреты:** v2 не создаёт `.env` с значениями. Существующий `.env` переносится в Keychain: `ai-secret import <project-scope> .env` — и файл удаляется (`.env.example` остаётся каталогом имён).
6. **Старые машины/проекты» (gsse, 3Dprint, 25000miles):** остаются на своих пинах до планового ре-онбординга; порядок — этот же раздел, по одному проекту за раз.

---

# Миграция с `emwoody-seo-cycle` на универсальный `seo-cycle` (историческое, v1)
