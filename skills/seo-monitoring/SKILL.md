---
name: seo-monitoring
description: Мониторинг (Phase 9): ежедневный pulse (GSC + Я.Вебмастер) в единую схему snapshot.json; GA4/PSI/Метрика Stats (`ga4-fetch`/`psi-fetch`/`metrika-fetch`) — отдельными командами в тот же snapshot через `snapshot-build.py --merge`; Метрика Logs cohorts (`cohorts`) — отдельный офлайн-отчёт вне snapshot. Ни одно из них не часть pulse. Потерянные ключи, AI-visibility (GEO), конкурентный бенчмарк. Используй когда просят «сними позиции», «что с трафиком», «обнови мониторинг» — отдельно или как стадию цикла.
---
...
**Cadence (единая, v2):** позиции собираются **ежедневно** — `seo-cycle pulse` (или `pulse --global` daily-джобом): fetch (GSC + Я.Вебмастер, только эти два источника — `scripts/pulse.py`) → snapshot → db-sync → position-progress + алерты. GA4, PSI (CWV) и Метрика Stats (посещения/behavior по URL) в `pulse` не входят — это отдельные команды (`seo-cycle run script ga4-fetch`, `seo-cycle run script psi-fetch`, `seo-cycle run script metrika-fetch`), совместимые с `snapshot-build.py --source ga4|psi|metrika --merge` (пишут в тот же `snapshot.json`, но отдельным шагом и по своему расписанию). Только Метрика Logs cohorts (`seo-cycle cohorts`, когортный анализ по TSV из `metrika-logs-fetch.py` — другой API той же Метрики) в snapshot вообще не пишет — это самостоятельный офлайн-отчёт. **Раз в 2 недели** поверх ежедневных снапшотов строится человеческий отчёт по `templates/monitoring-report.template.md`. Свежесть снапшота проверяет `seo-cycle doctor` (warn ≥3 дн., fail ≥7 дн.). Важно: Вебмастер-выгрузка — это `query_sample` (обычно топ-500 запросов, `sitewide: false`); все производные цифры (forecast, KPI, триггеры) наследуют этот потолок.

**Граница выборки (T-096).** Срез — это топ-N запросов **по показам** (`monitoring.sample.size`, дефолт 500), а не сайт: у gsse.ru 500 из ~2400 запросов ловят ~13 % кликов. Поэтому каждый отчёт (`pulse`, `position-progress`, `kpi-contract`, `seo-forecast`) печатает строку «выборка N из M запросов сайта» (M — из сводки источника; GSC и старые снапшоты его не отдают → «M неизвестно»), дельты топ-3/10/30 и алерт `top10_drop` считаются **только по пересечению** двух срезов, а запрос, выпавший из топ-N, помечается «выбыл из выборки», а не «потеря». Сырая разница составов печатается отдельно и подписана. Факты KPI (`monthly_organic_clicks`, `keywords_in_top10`) — «по выборке топ-N»; полная выгрузка сайта по умолчанию не делается (отдельное решение).
