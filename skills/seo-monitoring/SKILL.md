---
name: seo-monitoring
description: Мониторинг (Phase 9): ежедневный pulse (GSC + Я.Вебмастер) в единую схему snapshot.json; GA4/PSI — отдельными командами в тот же snapshot (`run script ga4-fetch`/`psi-fetch` + `snapshot-build.py --merge`), Метрика (`cohorts`) — отдельный офлайн-отчёт вне snapshot. Ни одно из трёх не часть pulse. Потерянные ключи, AI-visibility (GEO), конкурентный бенчмарк. Используй когда просят «сними позиции», «что с трафиком», «обнови мониторинг» — отдельно или как стадию цикла.
---
...
**Cadence (единая, v2):** позиции собираются **ежедневно** — `seo-cycle pulse` (или `pulse --global` daily-джобом): fetch (GSC + Я.Вебмастер, только эти два источника — `scripts/pulse.py`) → snapshot → db-sync → position-progress + алерты. GA4 и PSI (CWV) в `pulse` не входят — это отдельные команды (`seo-cycle run script ga4-fetch`, `seo-cycle run script psi-fetch`), совместимые с `snapshot-build.py --source ga4|psi --merge` (пишут в тот же `snapshot.json`, но отдельным шагом и по своему расписанию). Метрика (когорты, `seo-cycle cohorts`) в snapshot вообще не пишет — это самостоятельный офлайн-отчёт по visits TSV из `metrika-logs-fetch.py`. **Раз в 2 недели** поверх ежедневных снапшотов строится человеческий отчёт по `templates/monitoring-report.template.md`. Свежесть снапшота проверяет `seo-cycle doctor` (warn ≥3 дн., fail ≥7 дн.). Важно: Вебмастер-выгрузка — это `query_sample` (обычно топ-500 запросов, `sitewide: false`); все производные цифры (forecast, KPI, триггеры) наследуют этот потолок.
