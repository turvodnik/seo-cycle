...
8. Для приоритетов запускай `python3 ~/.claude/skills/seo-cycle/scripts/growth-roadmap.py --write` и начинай с `seo/setup/growth-roadmap.md`.
9. Для подробного first-run checklist запускай `python3 ~/.claude/skills/seo-cycle/scripts/setup-onboarding.py --write`; secret values вводит человек только в `.env`/кабинетах.
10. Для рекомендаций schedule запускай `python3 ~/.claude/skills/seo-cycle/scripts/automation-recommender.py --write`; `--apply` только после review, `--allow-schedules` только при явном разрешении.
11. Для детального intake используй `python3 ~/.claude/skills/seo-cycle/scripts/project-intake-wizard.py --interactive --write` или `--defaults --write`.
12. Для точечной настройки проекта используй `python3 ~/.claude/skills/seo-cycle/scripts/project-profile.py --write`; `--apply` только после review overlay/report.
13. Веди по фазам из `AGENTS.md`, но не шире task route и usage ledger.
