    ...
    steps.extend(
        [
            ...
            step(
                ...
                "Refresh readiness, validation, sources, setup blueprint, upgrade assistant, access-key assistant, task route, context pack, setup gap audit/questionnaire, answer-plan path, usage ledger, tool stack, growth roadmap, and automation recommendations.",
                commands=[f"python3 {skill_root()}/scripts/setup-control-plane.py {cfg_path} --write --task \"first SEO setup\""],
                proofs=["setup_control_plane", "setup_blueprint", "setup_matrix_csv", "upgrade_assistant", "upgrade_questionnaire_csv", "access_key_assistant", "context_pack_report", "setup_gap_audit_report", "setup_questionnaire", "setup_answer_plan", "latest_task_route", "latest_usage_report", "tool_stack_report", "growth_roadmap_report"],
                priority=90,
            ),
            step(
