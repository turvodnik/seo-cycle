...
def render_markdown(data: dict, project_name: str) -> str:
    ...
    if not pending:
        ...
        lines.append("")
        lines.append("Approve: `python3 ~/.claude/skills/seo-cycle/scripts/approval-gate.py approve <id>`")
    lines.append("")
    ...
    # Footer
    lines.append("---")
    lines.append(f"_Generated by `~/.claude/skills/seo-cycle/scripts/monthly-dashboard.py`. Обновлять по запросу или включить в cron._")
    return "\n".join(lines)
