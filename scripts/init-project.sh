...
python3 "$SKILL_ROOT/scripts/setup-control-plane.py" "$TARGET" --write --skip-intake >/dev/null 2>&1 \
    && echo "✓ setup control plane создан: launch-plan/spend-guard/setup/task-route/usage-ledger/tool-stack/growth-roadmap/onboarding + automation recommendations" \
...
echo "  9. Посмотри governance report:"
echo "     python3 ~/.claude/skills/seo-cycle/scripts/governance-report.py --format md"
echo "  10. Перед конкретной задачей построй low-token task route:"
...
echo "  11. Проверь/запиши расход токенов и платных инструментов:"
...
echo "  12. Сгенерируй и проверь рекомендации автоматизаций:"
...
echo "  13. Проверь рекомендуемый stack инструментов/доступов:"
...
echo "  16. Создай безопасный план автоматизаций:"
echo "     python3 ~/.claude/skills/seo-cycle/scripts/automation-plan.py --write --include-disabled"
echo "  17. В Claude Code/Codex: «давай запустим SEO-цикл для категории X»"
echo ""
