...
python3 "$SKILL_ROOT/scripts/setup-control-plane.py" "$TARGET" --write --skip-intake >/dev/null 2>&1 \
    && echo "✓ setup control plane создан: launch-plan/setup/task-route/usage-ledger/tool-stack/growth-roadmap/onboarding + automation recommendations" \
...
echo "  6. При необходимости доуточни подробный intake:"
echo "     python3 ~/.claude/skills/seo-cycle/scripts/project-intake-wizard.py --interactive --write"
echo "  7. Примени или обнови точечный project profile:"
...
echo "  8. Посмотри governance report:"
echo "     python3 ~/.claude/skills/seo-cycle/scripts/governance-report.py --format md"
echo "  9. Перед конкретной задачей построй low-token task route:"
...
echo "  10. Проверь/запиши расход токенов и платных инструментов:"
...
echo "  11. Сгенерируй и проверь рекомендации автоматизаций:"
...
echo "  12. Проверь рекомендуемый stack инструментов/доступов:"
...
echo "  15. Создай безопасный план автоматизаций:"
echo "     python3 ~/.claude/skills/seo-cycle/scripts/automation-plan.py --write --include-disabled"
echo "  16. В Claude Code/Codex: «давай запустим SEO-цикл для категории X»"
echo ""
