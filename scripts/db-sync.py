...
Источники (любой может отсутствовать — пропускается):
  ...
  JSON:
    ...
    seo/research/*/_usage.json   → api_usage (service, month, spent_usd, rows)
...
Использование:
    python3 ~/.claude/skills/seo-cycle/scripts/db-sync.py [--db PATH] [--root DIR]
"""
...
def sync_usage(conn, root: pathlib.Path) -> int:
    ...
    conn.execute("CREATE TABLE api_usage (service TEXT, month TEXT, spent_usd REAL, rows INTEGER)")
    n = 0
    for fp in glob.glob(str(root / "seo/research/*/_usage.json")):
        ...
        conn.execute("INSERT INTO api_usage VALUES (?,?,?,?)",
                     (service, u.get("month", ""), u.get("spent_usd", 0), u.get("rows", 0)))
        n += 1
    return n
