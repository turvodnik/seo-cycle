...
def main() -> int:
    ...
    index_lines = [
        ...
        "## Knowledge Flow",
        "",
        "1. `seo-cycle run script knowledge/wiki-refresh-all.sh` — обновляет wiki из WordPress и локальных отчётов.",
        "2. `seo-cycle run script knowledge/wiki-preflight` — проверяет дубли, правила проекта и риски перед правкой.",
        "3. `seo-cycle run script knowledge/wiki-context-pack` — даёт компактный контекст вместо чтения raw-артефактов.",
        "4. `seo-cycle run script knowledge/review-cluster-plan --write` — строит comparison/review-кандидаты только по реальным товарам и категориям.",
        "5. `seo-cycle run script knowledge/zvec-hybrid-index --build --write` — обновляет локальный hybrid/FTS индекс для поиска по wiki/vector.",
        "6. `seo-cycle run script knowledge/graphify-refresh.sh` — собирает curated corpus и строит Graphify-граф автоматически.",
        "",
        ...
        "```bash",
        "cd /path/to/project",
        "seo-cycle run script knowledge/graphify-refresh.sh",
        "```",
        "",
        "Что она делает сама:",
        "",
        ...
        "",
        "```bash",
        "GRAPHIFY_ANTIGRAVITY_ARGS=\"--max-files 0 --max-chars 26000 --per-file-chars 5000 --timeout 150\" seo-cycle run script knowledge/graphify-refresh.sh",
        "```",
        "",
