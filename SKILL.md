...
- Перед чтением подробных setup-отчётов открывай `seo/setup/context-pack.md`, затем `seo/setup/setup-questionnaire.csv` / `seo/setup/setup-gap-audit.md`, после заполнения CSV — `seo/setup/setup-answer-plan.md`, затем `seo/setup/launch-plan.md`: context pack ограничивает текущую задачу, questionnaire показывает что заполнить, answer plan показывает куда вручную внести review-safe ответы, launch-plan даёт первый экран проекта. Загружай `tool-stack`, `growth-roadmap`, `onboarding` и policy-файлы только если context pack/launch-plan указывают на нужный блок.
