...
- `seo/setup/setup-control-plane.md` — компактная readiness-сводка intake/profile/sources/governance/validation/tool-stack/spend-guard/growth-roadmap/onboarding/setup-blueprint/launch-plan/context-pack/setup-gap-audit/automation; создаётся `scripts/setup-control-plane.py --write`.
...
- Для первого запуска или handoff используй `scripts/setup-control-plane.py --write`: он собирает low-token readiness report, setup blueprint, context pack, setup gap audit и next actions без вывода секретов.
...
- Перед чтением подробных setup-отчётов открывай `seo/setup/context-pack.md`, затем `seo/setup/setup-blueprint.md`, затем `seo/setup/setup-questionnaire.csv` / `seo/setup/setup-gap-audit.md`, после заполнения CSV — `seo/setup/setup-answer-plan.md`, затем `seo/setup/launch-plan.md`: context pack ограничивает текущую задачу, blueprint показывает точечную матрицу проекта, questionnaire показывает что заполнить, answer plan показывает куда вручную внести review-safe ответы, launch-plan даёт первый экран проекта. Загружай `tool-stack`, `growth-roadmap`, `onboarding` и policy-файлы только если context pack/blueprint/launch-plan указывают на нужный блок.
