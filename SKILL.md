...
- Для черновика используй `copywriting_playbook` и `writer_prompt_packet` из `page-outline-v2` как основной low-token handoff: они задают reader state, tone contract, angle stack, draft sequence, forbidden actions и acceptance gate. Raw research, CSV и full SERP загружай только если source slot или fact-check queue просит конкретный источник.
