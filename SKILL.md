...
- `seo/setup/project-journey.md`, `seo/setup/project-journey.json` и `seo/setup/project-journey-checklist.csv` — автоматический путь от старта до цели: текущая стадия, missing artifacts, blockers, next command, exit criteria и action plan; создаётся `scripts/project-journey.py --write`.
...
- `<package>/semantic-core.cleaned.csv`, `<package>/semantic-core.rejected.csv`, `<package>/semantic-core.resynced.csv`, `<package>/entity_coverage.jsonl`, `<package>/content-plan.orphan-backlog.csv`, `<package>/serp-validation-plan.csv`, `<package>/spoke-opportunities.csv`, `<package>/entity-graph-quality.md/json` — repair layer для findings research package gate: очистка ядра, ресинхрон URL/cluster IDs, синхронизация entity map, агрегация Google NLP, orphan backlog, план SERP-проверки, phase-2 spokes и качество entity graph; создаётся `semantic-core-clean.py`, `semantic-core-resync.py`, `entity-map-sync.py`, `google-nlp-aggregate.py`, `orphan-url-resolver.py`, `serp-validation-plan.py`, `spoke-opportunity-audit.py`, `entity-graph-quality.py`.
- `seo/research-package/page-outlines-v2/*.md/json` или `<package>/page-outlines-v2/*.md/json` — глубокие page briefs уровня H2/H3: computed word count, H3 word-count allocation, intro/conclusion brief, SEO meta, Key Takeaways, FAQ, visual plan, section bridges, writer handoff, `copywriting_playbook`, `writer_prompt_packet`, copywriting details, source slots, acceptance criteria, entities, keywords, visuals, copywriter notes, Answer Units, evidence required, schema, internal links, synthetic prompts и E-E-A-T no-fabrication guard; создаётся `scripts/page-outline-v2.py <package> --page "<url|cluster|keyword>" --write`, массово: `--all-mvp --write` или `--priority P1 --write`.
- `seo/research-package/page-outline-quality.md/json` или `<package>/page-outline-quality.md/json` — quality gate для page briefs: word-count drift, H3/H2 word-count mismatch, SERP/page-type lock, intro/conclusion, SEO meta, Key Takeaways, FAQ, writer handoff, copywriting playbook, writer prompt packet, revision checklist, fact-check queue, schema, internal links, Answer Units, source slots, acceptance criteria, evidence, entity orphans, section bridges, visuals, trust limits, synthetic prompts и no-fabricated-expertise; создаётся `scripts/page-outline-quality.py <package> --write`.
- `<draft>.draft-quality-gate.md/json` — gate для markdown-черновика против `page-outline-v2`: missing H2/H3, unsafe first-person expertise, missing internal links, missing proof/source slots и FAQ mismatch; создаётся `scripts/draft-quality-gate.py <draft.md> --outline <outline.json> --write`.
...
- Для первого запуска или handoff используй `scripts/setup-control-plane.py --write`: он собирает low-token readiness report, setup blueprint, upgrade assistant, access-key assistant, project journey, context pack, token-waste audit, Perplexity/NotebookLM health, setup gap audit и next actions без вывода секретов.
...
- Перед Perplexity/NotebookLM evidence work запускай `scripts/perplexity-health.py --write` и `scripts/notebooklm-health.py --write`. Perplexity использует persistent browser/app сессию, если она доступна, API остаётся optional/paid и disabled by default; NotebookLM — curated expert evidence only, не volume/KD/ranking signal. Сбор делай через `scripts/perplexity-collect.py --topic "<тема>" --write` и `scripts/notebooklm-source-pack.py --topic "<тема>" --export-file <file> --write`: raw ответы и transcripts сохраняются в `seo/research/raw/`, downstream prompts получают только `seo/research/distillates/*/latest-summary.md` + citations, vector-связи пишутся в `seo/research/vector/source_pack.jsonl`.
...
- Если package quality или внешний аудит нашли repair findings, сначала запускай соответствующий repair layer: `semantic-core-clean.py`, `semantic-core-resync.py`, `entity-map-sync.py`, `google-nlp-aggregate.py`, `orphan-url-resolver.py`, `serp-validation-plan.py`, `spoke-opportunity-audit.py`, `entity-graph-quality.py`. После repair повторяй quality gate до генерации новых page briefs.
- Для MVP/P1 страниц после прохождения research-package gate запускай `scripts/page-outline-v2.py <package> --all-mvp --write` или `--priority P1 --write`. Это обязательный мост между макро-пакетом (semantic core, clusters, site architecture) и writing phase: не писать/публиковать страницу только по скелетному `mvp-page-briefs.md`.
- Сразу после `page-outline-v2.py` запускай `scripts/page-outline-quality.py <package> --write --format markdown` и следуй его action plan. Writing/design/schema/publishing blocked, пока `page-outline-quality.json` отсутствует или содержит critical findings; `project-journey.py --write` тоже покажет этот стопор как текущий шаг.
- Для черновика используй `copywriting_playbook` и `writer_prompt_packet` из `page-outline-v2` как основной low-token handoff: они задают reader state, tone contract, angle stack, draft sequence, forbidden actions и acceptance gate. После черновика запускай `draft-quality-gate.py <draft.md> --outline <outline.json> --write`; raw research, CSV и full SERP загружай только если source slot или fact-check queue просит конкретный источник.
