...
- После first-run или изменения policy запускай `scripts/setup-gap-audit.py --write` и закрывай missing fields через `seo/setup/setup-questionnaire.csv` / `seo/setup/setup-gap-audit.md` до широких циклов, платных API/LLM, массового local/ecommerce или automations.
...
- Перед чтением подробных setup-отчётов открывай `seo/setup/context-pack.md`, затем `seo/setup/setup-questionnaire.csv` / `seo/setup/setup-gap-audit.md` и `seo/setup/launch-plan.md`: context pack ограничивает текущую задачу, questionnaire показывает что заполнить и куда внести ответ, launch-plan даёт первый экран проекта. Загружай `tool-stack`, `growth-roadmap`, `onboarding` и policy-файлы только если context pack/launch-plan указывают на нужный блок.
