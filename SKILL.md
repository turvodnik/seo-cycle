...
- Если package quality или внешний аудит нашли repair findings, сначала запускай единый repair layer: `scripts/research-package-repair.py <package> --write`. Если нужен точечный режим, бери конкретные команды из `research-package-action-plan.md`: `semantic-core-clean.py`, `semantic-core-resync.py`, `entity-map-sync.py`, `google-nlp-aggregate.py`, `orphan-url-resolver.py`, `serp-validation-plan.py`, `spoke-opportunity-audit.py`, `entity-graph-quality.py`. После repair повторяй quality gate до генерации новых page briefs.
