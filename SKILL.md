...
- `seo/research-package/page-outlines-v2/*.md/json` или `<package>/page-outlines-v2/*.md/json` — глубокие page briefs уровня H2/H3: computed word count, H3 word-count allocation, `metrics_rollup` из semantic core (volume/clicks/impressions/priority/top supporting keywords), intro/conclusion brief, SEO meta, Key Takeaways, FAQ, visual plan, section bridges, writer handoff, `copywriting_playbook`, `writer_prompt_packet`, copywriting details, source slots, acceptance criteria, entities, keywords, visuals, copywriter notes, Answer Units, evidence required, schema, internal links, synthetic prompts и E-E-A-T no-fabrication guard; создаётся `scripts/page-outline-v2.py <package> --page "<url|cluster|keyword>" --write`, массово: `--all-mvp --write` или `--priority P1 --write`; `--archive-legacy-briefs` переносит дубли старых briefs только по явному флагу.
