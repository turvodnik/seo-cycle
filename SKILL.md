...
- `seo/setup/setup-gap-audit.md` и `seo/setup/latest-setup-gap-audit.md` — readiness score и missing fields по рынку, бизнесу, local/ecommerce, инструментам, budget/subscriptions, spend guard и automations; создаётся `scripts/setup-gap-audit.py --write`.
...
- `seo/setup/setup-control-plane.md` — компактная readiness-сводка intake/profile/sources/governance/validation/tool-stack/spend-guard/growth-roadmap/onboarding/launch-plan/context-pack/setup-gap-audit/automation; создаётся `scripts/setup-control-plane.py --write`.
...
- Для первого запуска или handoff используй `scripts/setup-control-plane.py --write`: он собирает low-token readiness report, context pack, setup gap audit и next actions без вывода секретов.
...
- После first-run или изменения policy запускай `scripts/setup-gap-audit.py --write` и закрывай missing fields в `seo/setup/setup-gap-audit.md` до широких циклов, платных API/LLM, массового local/ecommerce или automations.
...
- Перед чтением подробных setup-отчётов открывай `seo/setup/context-pack.md`, затем `seo/setup/setup-gap-audit.md` и `seo/setup/launch-plan.md`: context pack ограничивает текущую задачу, gap audit показывает недозаполненные поля, launch-plan даёт первый экран проекта. Загружай `tool-stack`, `growth-roadmap`, `onboarding` и policy-файлы только если context pack/launch-plan указывают на нужный блок.
