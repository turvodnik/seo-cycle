...
- `seo/research-package/page-outlines-v2/*.md/json` или `<package>/page-outlines-v2/*.md/json` — глубокие page briefs уровня H2/H3: computed word count, SEO meta, Key Takeaways, FAQ, visual plan, section bridges, writer handoff, entities, keywords, visuals, copywriter notes, Answer Units, evidence required, schema, internal links, synthetic prompts и E-E-A-T no-fabrication guard; создаётся `scripts/page-outline-v2.py <package> --page "<url|cluster|keyword>" --write`, массово: `--all-mvp --write` или `--priority P1 --write`.
- `seo/research-package/page-outline-quality.md/json` или `<package>/page-outline-quality.md/json` — quality gate для page briefs: word-count drift, SERP/page-type lock, SEO meta, Key Takeaways, FAQ, writer handoff, fact-check queue, schema, internal links, Answer Units, evidence, entity orphans, section bridges, visuals, trust limits, synthetic prompts и no-fabricated-expertise; создаётся `scripts/page-outline-quality.py <package> --write`.
