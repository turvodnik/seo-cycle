...
- `seo/setup/setup-control-plane.md` — компактная readiness-сводка intake/profile/sources/governance/validation/tool-stack/spend-guard/growth-roadmap/onboarding/launch-plan/context-pack/automation; создаётся `scripts/setup-control-plane.py --write`.
...
- Для первого запуска или handoff используй `scripts/setup-control-plane.py --write`: он собирает low-token readiness report, context pack и next actions без вывода секретов.
...
- Перед чтением подробных setup-отчётов открывай `seo/setup/context-pack.md`, затем `seo/setup/launch-plan.md`: context pack ограничивает текущую задачу, launch-plan даёт первый экран проекта. Загружай `tool-stack`, `growth-roadmap`, `onboarding` и policy-файлы только если context pack/launch-plan указывают на нужный блок.
