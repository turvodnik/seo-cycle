...
- Перед Perplexity/NotebookLM evidence work запускай `scripts/perplexity-health.py --write` и `scripts/notebooklm-health.py --write`. Perplexity использует persistent browser/app сессию, если она доступна, API остаётся optional/paid и disabled by default; NotebookLM — curated expert evidence only, не volume/KD/ranking signal. Raw ответы и transcripts сохраняй в `raw/`, downstream prompts получают только distillates + citations.
