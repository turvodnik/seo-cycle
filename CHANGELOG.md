...
- **F-2 — денежный шлюз закрыт и на пути через HTTP(S)-прокси.** При заданной
  ...
  **рекомендует** ходить в DataForSEO через прокси для основного региона
  ...
  (5 тестов: патч установлен, неармированный платный хост через туннель
  отказывает, свободный хост не трогается, армированный хост проходит,
  регистр/точка на конце не обходят проверку) — мутация (снятие
  ...
  граница «Does NOT buy» больше не молчит про прокси как отдельный
  неназванный путь — он закрыт.
- **F-3 — `client-report.py` отказывает вместо отчёта с именем каталога
  ...
  → пусто.
- **F-4 — исключение мастеров настройки: текст приведён к факту.** Обоснование
  (`setup-onboarding.py`/`project-intake-wizard.py` обязаны работать без
  готового конфига — T-090, F-8) остаётся законным и в этом пакете не
  тронуто. QA нашёл, что текущая формулировка исключения в CHANGELOG/тестах
  называет 2 мастера + 5 легитимных «без конфига» команд + 3 файла, не
  читающих `project.*` — то есть **10** поимённо названных исключений из
  `require_section()`, а фактическая матрица QA (159 скриптов × 7 форм,
  1113 ячеек) нашла **59 GREEN у 12 скриптов**, а не у трёх — расхождение
  в счёте, не в списке. Повторная репродукция обоих мастеров (та же команда,
  что в отчёте QA) подтверждена лично в этом пакете: `setup-onboarding.py
  --write` на `just a string` → `EXIT=0`, 6 файлов, `Project: ? (?)` — байт
  в байт как в отчёте. Полный пересчёт всех 1113 ячеек этим пакетом не
  повторялся (непропорционально задаче привести ТЕКСТ к факту, а не заново
  ставить диагноз) — счёт «12, не 3» принимается по независимому QA
  четвёртого прогона как источник истины, не по памяти; частично
  верифицированная часть (два мастера) сошлась побайтово. Из двенадцати
  QA называет как минимум четыре защитимых конкретно (`secret-scan.py`,
  `skill-manifest-validate.py`, CLI без подкоманды печатает `--help`, два
  библиотечных модуля без `__main__`) и явно отделяет их от двух мастеров,
  которые пишут документ с подставной личностью проекта — то есть та же
  формулировка ЧЕСТНО остаётся «два мастера пишут фиктивный документ», но
  общее число «зелёных» в матрице — 12, и этим пакетом зафиксировано так, а
  не как «три», нигде в дереве буквально не встречающееся, но подразумевавшееся
  предыдущими раундами T-090/T-093 как «всё, что не require_section — уже
  названо и закрыто списком».
