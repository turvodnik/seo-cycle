...
- Все три места сведены к одной функции `fetch_tags_or_report()` (force+prune-tags,
  проверка кода возврата, честное сообщение о причине). Обойти её нельзя — других путей
  получить теги в `install.sh` не осталось (перепроверено `grep` по всем вызовам `git
  fetch`/`ls-remote`/`rev-parse` в файле).
...
`<notebook-id>`, как в тестовых фикстурах.
