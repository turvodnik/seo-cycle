...
- **Частичный перепин без отчёта.** `attach_project()` завершает свои
  ...
  проекта в подпроцесс, печатает «перепинено / не тронуто / упало» и
  завершается ненулевым кодом при любом падении. Заодно закрыт смежный
  дефект: `mkdir -p .agents/external`, проваливавшийся молча (тот же баш-
  трюк — errexit не прерывает функцию, вызванную из условия `if`), позволял
  дописать «✓ lock» даже когда симлинк так и не был создан.
...
- Тесты: `tests/test_installer_contracts.py` —
  ...
  `OrphanedWorktreeRefusesTest` (мутационные негативные контроли на все три
  фикса).
