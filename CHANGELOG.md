...
`SEO_CYCLE_AI_SECRET=<путь>`. Тексты `AI_SECRET_MISSING`, докстрингов
лончера/`auth-assistant`, `docs/troubleshooting.md` приведены к факту.
...
переменная выбирает брокер; переменная в никуда → rc 3. Мутация обратно на
`shutil.which` — 4/4 красных.
