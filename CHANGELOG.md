...
контракт rc/`ERROR:` для этого случая. Тесты: `tests/test_project_journey.py`
расширен на 5 (`StageTitlesAreRussianTest`, `StatusHeaderTest`) — заголовки
без латиницы кроме `NeuronWriter`, шапка — первые три строки, `Дальше:`
существует в `COMMANDS`/`EXTRA_COMMANDS`. Логика стадий/блокеров не
менялась — только тексты и шапка.
