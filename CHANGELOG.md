...
и `config/project.template.yaml` обещали синхронизацию с
`cost_controls.dataforseo.monthly_usd_cap`. Решение Vladimir 04.09 (записано в
`optimize/SPEC-seo-cycle-fixes.md`, п. 13) сняло действовавший до этого запрет на
правку поведения DataForSEO-кода — денежный баг чинится до публичного тега.
...
- **`effective_budget()`** — читает `cost_controls.dataforseo.monthly_usd_cap` из
  `seo-cycle.yaml` проекта (через `seo_cycle_core.config`) и берёт минимум с
...
- ⚙️ **Не в этом тикете**: та же ложная строка «Мировые данные, вкл. ЕС и РФ» и
  тот же пример `--location 2643743` повторяются в `GUIDE.md` (RU и EN секции) —
  вне `depends_on`/файлового скоупа T-059, наблюдение передано в «Результат» тикета
  для отдельной правки (кандидат — тикет документации).
