...
### Fix: седьмой денежный стоп (`google-nlp-audit.py`) + единая точка write-ahead для всех платных клиентов (T-089, F-1)
...
- **`google-nlp-audit.py`** переведён на write-ahead: `add_usage()`/
  ...
  здесь известна заранее (по длине текста), так что, в отличие от
  DataForSEO/SpyFu, никакого отдельного состояния «сумма неизвестна» не
  нужно — write-ahead-запись уже финальна.
- **`seo_cycle_core/spend_guard.py`** (новый модуль) — единая точка, через
  которую обязан пройти любой платный вызов у всех шести клиентов:
  `@guarded_spend` декорирует функцию, которая реально стучится в сеть
  (`call()`/`call_feature()`/`apply_direct()`/`live_fetch()`/`gaql_search()`);
  вызов вне `armed_spend(write_ahead)` поднимает `SpendNotArmedError` —
  структурный отказ, а не подразумеваемая дисциплина код-ревью. `armed_spend()`
  сама не пишет ничего на диск (это остаётся специфичным для клиента), но
  требует, чтобы переданный `write_ahead` реально отработал и вернул `True`,
  ДО того как разрешить платный вызов внутри `with`.
- Честно про границу: `@guarded_spend`/`armed_spend()` не заставляют автора
  восьмого клиента вообще их применить — забытый декоратор на новом файле
  по-прежнему тихий пробел, найти который снова придётся код-ревью/QA. Что
  меняется структурно: если декоратор ПРИМЕНЁН (а на все шесть существующих
  клиентов он применён этим тикетом), обойти его вызовом мимо
  `armed_spend()` — уже не «забыли одну строку записи», а гарантированный
  `SpendNotArmedError` при первом же прогоне, без исключений по типу клиента.
  То есть седьмой случай (write-ahead расставлен вручную, по перечню)
  закрыт: у существующих клиентов не может тихо «отвалиться» защита при
  рефакторинге рядом — попытка вызвать платную функцию без
  предшествующей успешной записи в леджер падает сама, а не полагается на
  то, что кто-то не забыл переставить строки в нужном порядке.
