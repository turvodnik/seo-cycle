...
- **Второй круг после гейта §6** (два независимых ревью нашли, что «тип
  ...
  `json.loads` штатно принимал литерал `NaN` из леджера или из ответа API
  (`"cost": NaN`), и `nan >= budget` было всегда ложно — денежный стоп
  отключался навсегда и без единого сообщения. `effective_budget()` тем же
  правилом отказывает (`sys.exit`) на мусорном `monthly_usd_cap` (строка,
  список, `NaN`, отрицательное) и на битом YAML конфига — раньше опечатка в
  лимите тихо откатывала на `--budget`. `save_usage()` — новый тест на
  атомарность (`os.replace`), которую до этого не проверял ни один тест.
