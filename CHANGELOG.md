...
- **`seo_cycle_core.config.load_config()`** — теперь отказывает (код 2),
  когда ключ `project` присутствует и не является мэппингом (`null`,
  строка, список, число) — закрывает класс разом для ВСЕХ текущих и
  будущих вызывающих `load_config()`, включая ~13 пользователей
  `require_config` и 14 vnext-аудит скриптов, без правки каждого файла по
  отдельности. Срабатывает только когда ключ `project` присутствует и
  испорчен — конфиг, вообще не упоминающий `project`, не трогается (это
  зона `require_section`).
- **~49 скриптов**, читавших свой главный конфиг через нестрогий
  ...
  есть, но пуст" и там, где правило A само по себе не сработало бы (пустой
  файл не содержит ключа `project`, которому можно было бы быть испорченным).
- `ai-bot-access-check.py` — импортировал нестрогий `load_yaml` через
  ...
  путём.
- `technical-site-audit.py` — тот же перевод `load_yaml` → `load_config`.
- `geo-citation-log.py` — раньше находил путь к конфигу, но НИКОГДА не
  ...
  файла) — добавлен `load_config(cfg_path)` как чистая проверка
  валидности (возврат отброшен намеренно, скрипту не нужно ни одно поле).
...
- Поведение на исправном конфиге не изменилось: побайтовое сравнение
  stdout/stderr/кода возврата/списка созданных файлов на выборке из 14
  скриптов разных типов до/после (worktree на `origin/main` до правки vs
  ветка после), различия — только временные метки/пути временных
  каталогов.
- Тесты: `tests/test_t093_config_null_remainder.py` — по одному тесту на
  (скрипт × форма плохого конфига), 370 тестов, каждый запускает
  реальный CLI-скрипт подпроцессом (не импорт хелпера) и проверяет и код
  возврата, и список созданных файлов. Мутация (откат проверки формы
  `project` в `load_config`) убивает 176 тестов; откат отдельной миграции
  `technical-site-audit.py` обратно на `load_yaml` убивает 2 теста на
  пустом/комментарийном конфиге — оба подтверждают, что тесты проверяют
  продукт, а не отсутствие трейсбека.
- Известный остаток (честно, не выдан за закрытый): матрица не покрывает
  формы «не-UTF-8» и «`{{{`» отдельно для каждого из ~99 файлов реестра —
  общий парсер (`load_yaml`/`load_config`) их уже отказывает с координатой
  (T-067/T-090), это специфично проверено только на выборке; `dataforseo-fetch.py`
  не верифицирован (см. выше).
