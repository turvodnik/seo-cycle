...
- **`seo_cycle_core/__init__.py`** теперь импортирует `spend_guard` как
  ...
  как только файл импортирует хоть что-то из `seo_cycle_core` (что уже
  делают все восемь платных клиентов ради config/usage_ledger/ads).
...
- **`tests/helpers/t089_signal_target.py`** — харнесс для `ads-apply.py`/
  ...
  `ledger_record()`, продуктовый код в девяти из 24 ячеек матрицы не
  исполнялся вовсе — проверял харнесс, не продукт).
