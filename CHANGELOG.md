...
- `psi-fetch.py` в batch-режиме больше не проглатывает упавшие URL молча:
  ...
  собралось ничего.
- `monthly-dashboard.py`: путь мониторинга берётся из `monitoring.path`
  конфига (дефолт `seo/monitoring`, fallback на v1-раскладку
  `seo/09-monitoring`), маска `*snapshot*.json` вместо `*-snapshot.json` —
  раньше блок «Последний Monitoring Snapshot» ВСЕГДА писал «не найден» для
  v2-раскладки (I-060: снапшот кладёт дату ПОСЛЕ слова snapshot). Если
  снапшотов действительно нет — сообщение называет, где искали.
- `site-crawl.py`: битый HTML больше не теряет ссылки/title/h1 страницы без
  следа — счётчик `parse_errors` в отчёте и debug-лог с трейсбеком; сводка
  называет число пропущенных страниц.
