...
**Ломающее изменение (уточняет T-108).** `ai-secret` больше не ищется в
`PATH`: `find_ai_secret()` берёт `~/.local/bin/ai-secret` (канонический
путь установки брокера), другой путь — только через явную переменную
`SEO_CYCLE_AI_SECRET`; переменная, указывающая в никуда, — честный rc 3
«секреты не подключены», без отката на дефолт или `PATH`. Причина:
`auth set`/`auth login` и `gbp-oauth-helper.py` передают значение секрета
брокеру по stdin, а лончер делает `exec` на него — каталог раньше в `PATH` с
файлом `ai-secret` получал бы значение (политика секретов: `PATH` и
проектные бинарники недоверены). Кому это ломает: установкам, где брокер
лежит не в `~/.local/bin` и находился только через `PATH`, — задать
`SEO_CYCLE_AI_SECRET=<абсолютный путь>`. Переменная действует только из
окружения процесса: `env_chain` не пропускает `SEO_CYCLE_AI_SECRET` и `HOME`
из проектного `.env`/`env.global` (`PROCESS_ONLY_NAMES`) — иначе
склонированный репозиторий мог бы через свой `.env` назначить получателя
значений `auth set`; относительный путь в переменной отвергается (резолвился
бы от cwd проекта). Тексты `AI_SECRET_MISSING`, докстрингов
лончера/`auth-assistant`, `INSTALL.md` (Шаг 4), `docs/troubleshooting.md`,
`docs/vps-deployment.md` приведены к факту.
Тесты канона подключают заглушку брокера только через эту переменную под
пустым `HOME` (настоящий Keychain недостижим); четыре сторожа
`BrokerResolutionTest`: заглушка первой в `PATH` без переменной → rc 3 и
пустой лог; сентинел в `PATH` + брокер по канону → сентинел не вызван;
переменная выбирает брокер; переменная в никуда → rc 3; плюс враждебный
checkout (`.env` называет свой `ai-secret`) → вызван канонический брокер,
`env_chain` без имён из файлов, относительная переменная → rc 3. Мутации:
обратно на `shutil.which` — 4/4 красных; без фильтра в `env_chain` —
сентинел из `.env` получает `set`; без проверки абсолютного пути — красный.

**Fix.** `auth list`: неизвестный `tier` у провайдера в `PROVIDERS` давал
голый `KeyError` (rc 1 с трассировкой). Метки вынесены в `TIER_LABELS`,
`cmd_list` проверяет все провайдеры до ветки формата и отвечает rc 2 с
именем провайдера, значением и допустимым списком; тест
`test_every_provider_tier_is_known` сторожит таблицу (мутация
`"tier": "foo"` — красный).

**Docs.** `skills/seo-monitoring/SKILL.md` называл раскладку v1
`09-monitoring/…`; факт — `seo/monitoring/webmaster-snapshot-<дата>.json`
(`pulse`, каталог `monitoring.path`, дефолт `seo/monitoring`, оттуда же
читают `doctor`/`status`). Для `snapshot-build.py` явно сказано задавать
`--output` (его собственный дефолт — ещё старая раскладка, см. наблюдения
фикс-прохода). README: `status` показывает эскалации, не «approvals».

**Tests.** Ветка `--store-scope` `gbp-oauth-helper.py` (после обмена кода)
получила тест: обмен подменён, код через `--redirect-url`, брокер —
заглушка; в логе заглушки `set testproj GBP_OAUTH_REFRESH_TOKEN`, значение
дошло по трубе, в `.env` только `GBP_TOKEN_MINTED_AT=<дата>`, значения нет в
stdout/stderr/.env/логе (мутация «вернуть `upsert_env_var` с токеном» —
красный). `tests/test_registry_not_polluted.py` требует
`result.wasSuccessful()` у прогона сторожевых тестов — упавший guarded-тест
больше не даёт вакуозный «реестр не тронут».
...
### Fix: клиентский отчёт без внутренних команд, с полоской топ-10 и словарём (T-106, H8)

`client-report.py`: в пустом состоянии отчёт клиенту больше
не содержит операторскую подсказку «запустите forecast/kpi/sync/db-sync» —
текст заменён на нейтральное «данные появятся после ближайшего среза», а
сама подсказка для оператора теперь печатается в stderr при генерации.
Раздел «Видимость в поиске» при ≥2 снапшотах в `seo.db` собирает полоску
топ-10 по срезам (`html_report.bar()`, тот же CSS-инлайн-компонент, что уже
использует `position-progress.py` — переиспользование, не дублирование) —
только в HTML, без внешних ресурсов. В конце непустого отчёта (md и HTML)
добавлен блок «Словарь терминов» (топ-10, показы, клики, позиция, срез — по
одной строке). Тесты `tests/test_client_report.py` расширены: проверка на
отсутствие подстрок `запустите|seo-cycle |python3` в HTML (на HEAD было 1
совпадение на фикстуре с пустым forecast, теперь 0), наличие полоски и
словаря на фикстуре с 2 снапшотами, инвариант «нет `http` в HTML»
(негативный контроль — вставка `<script src=http…>` в копию шаблона красит
тест — проверено вручную, не закоммичено как отдельный тест).

**Фикс-раунд 1 (гейт, 🟡).** `html_report.bar()` (в отличие от `html_page()`)
не экранирует свой `color=` перед подстановкой в атрибут `style="..."` —
враждебный `agency.accent_color` из конфига ломал атрибут и вставлял живой
`<script src="http…">` в полоску топ-10 (2 совпадения на фикстуре с 2
снапшотами), пробивая тот же инвариант «без внешних ресурсов», который
тикет объявляет неприкосновенным. Добавлена `safe_accent_color()`:
значение проверяется по `^#[0-9a-fA-F]{3,8}$`, иначе — откат на
`DEFAULT_ACCENT` с предупреждением в stderr. Тест
`test_hostile_accent_color_cannot_inject_a_script` (мутация — откат
валидации красит тест: 0 → 2 живых `<script src="http`).
...
Значения кривой (1–10, `CTR_11_20`, `CTR_BEYOND`) не менялись. Одно узкое
правило поведения сменилось для forecast: позиция в диапазоне `(0; 0.5]`
(bucket 0) раньше через локальный `ctr_for()` получала `CTR_11_20` (0.01),
теперь через `expected_ctr()` ядра — `CTR_BEYOND` (0.002); на живых данных
недостижимо (все писатели позиций дают ≥1 или трактуют «нет позиции» иначе,
SQL-фильтр `position > 0` такую строку пропустит только из рукописной
записи в `seo.db`), закреплено тестом `test_ctr_curve_bucket_zero_uses_beyond`
в ядре. Тесты: `tests/test_forecast_kpi.py::test_single_ctr_curve_source`
...
### Change: мастер — 7 вопросов по умолчанию (T-107, H3)

`init-project.sh` задавал 23 вопроса подряд, 9 из них — про картинки (16:9,
WebP, ширина 1200, quality 86); первый контакт нового человека = допрос про
WebP (отчёт `2026-09-15-seo-cycle-people-and-quality.md` §4.2/§4.5). Теперь
по умолчанию — 7 обязательных вопросов (имя проекта, домен, project_type,
CMS, язык/регион — определяет и `region_profile`, и включённые поисковые
движки, governance/бюджет-профиль, monthly paid API budget) и один
завершающий переключатель «Уточнить картинки, расписания и подробный intake
сейчас? [y/N]»; при «y» задаются все прежние вопросы (бренд, LLM-бюджет,
automation mode, schedules, 9 вопросов про картинки, подробный intake) в
прежнем порядке — ни один вопрос не удалён, только перенесён за
переключатель. Значения дефолтов из `config/project.template.yaml` не
менялись — при «N» (Enter) конфиг получает ровно те же дефолты, что и
раньше. Финальный экран — 17 «следующих шагов» заменён на три строки
(`seo-cycle validate` / `doctor` / `status`) + ссылка на
`seo/setup/onboarding-playbook.md` (проектный путь — единственный, который
гарантированно существует сразу после генерации; INSTALL.md уже называет
его «подробным файлом первого запуска»). Тесты: `tests/test_init_project.py`
(`WizardSevenQuestionsTest` — default-прогон даёт ровно 8 промптов до
генерации и `validate` rc=0, инсценировка сломанного `project.domain` даёт
rc≠0, «y» на переключателе открывает вопросы про WebP);
`tests/test_intake_wizard_tty.py` (T-099) — ответы-последовательности
пересчитаны под новый порядок вопросов, логика не менялась.

### Change: вывод status/journey по-русски, шапка из трёх строк (T-105, H5)

`seo-cycle status` смешивал русский дашборд со стадиями `project-journey.py`
на английском («Setup foundation», «Technical baseline» и остальные 10
заголовков) — теперь все 12 заголовков и описаний стадий (`title`/
`objective` в `scripts/project-journey.py`; `id` для машин остался
английским) переведены на русский. `status` печатает новую шапку первыми
тремя строками: `Срез: ок/предупреждение/просрочен (файл · N дн.)` ·
`Стадия: <название> (N из 12)` · `Дальше: <одна команда>` — свежесть среза
монитора и готовность стадии setup теперь в разных строках, а не в одной
общей оценке. Подробный вывод `project-journey.py` ниже не менялся.
`Дальше:` берёт первую `seo-cycle `-команду из `next_commands` текущей
стадии (fallback — `seo-cycle journey`, если ни одна не CLI-shaped, как у
`research_architecture`). Шапка строится через прямой импорт
`project-journey.py` (`importlib`, как в `page-outline-v3.py`); при сбое
(например, `project:` в конфиге — не блок) падает обратно на прежний
англоязычный `- снапшот: ... ok/warn/ПРОСРОЧЕН`, а не ломает существующий
контракт rc/`ERROR:` для этого случая (сбой печатает одну строку в stderr,
не глушится молча). `--research-package <путь>` разбирается из
passthrough-аргументов и доходит до шапки — раньше флаг видело только тело
`project-journey.py` (делегированный subprocess), и шапка с телом могли
называть разные стадии в одном выводе. Тесты: `tests/test_project_journey.py`
расширен на 9 (`StageTitlesAreRussianTest`, `StatusHeaderTest`,
`HeaderSelectorUnitTest`) — заголовки без латиницы кроме `NeuronWriter`,
шапка — первые три строки с точным `assertEqual` на стадии 1, выбор
`Дальше:`/номер стадии проверены вызовом реальных `_pick_next_command()`/
`_format_stage_line()` на всех 12 стадиях (не копией правила внутри теста),
`--research-package` — шапка и тело называют одну стадию. Логика
стадий/блокеров не менялась — только тексты и шапка.
...
(pulse вернул пустой срез; `doctor` печатает `agy: missing`, но это не то,
что красит exit-код; `loop` эскалировал — что делать; проекта нет во вкладке
`web`; PDF отчёта без Chrome; ключ есть в Keychain, а `seo-cycle` его не
видит) — добавлен первый раздел «Оператору: шесть частых ситуаций» с
симптомом → причиной → командой по каждому, все команды проверены
`--help`. `SKILL.md:12` обещал Gemini как рантайм — кода под него нет
(`SEO_RUNTIME=claude|codex|auto`); фраза сужена до факта, Gemini/Grok прямо
помечены «не поддерживаются как рантайм». `skills/seo-monitoring/SKILL.md`
description и cadence обещали, что `pulse` тянет Метрику/GA4/PSI — код
(`scripts/pulse.py`) собирает только GSC и Я.Вебмастер; текст сужен до факта,
Метрика/GA4/PSI перечислены как отдельные команды (`cohorts`, `run script
ga4-fetch`, `run script psi-fetch`). `GUIDE.md:18` обещал «нарезку по модулям
...
начинать день. `command_overview()` теперь группирует `COMMANDS` (поле
`group`, дефолт «Прочее») и статически прописанные команды (`gate`, `status`
и др. из `EXTRA_COMMANDS`) в шесть разделов: «Сегодня» (pulse · status · web,
первой и в этом порядке), «Контент», «Деньги», «Клиенты», «Настройка»,
«Прочее» (алфавитом). Неизвестное значение `group` — `ValueError` при
построении `--help`, не тихое попадание в «Прочее». README получил блок
«Утро оператора: три команды» сразу после заголовка TL;DR, раньше первого
упоминания vendor/MCP/Graphify. `auth-assistant.py list` печатает сверху
строку «Для ежедневного среза достаточно: Яндекс.Вебмастер **или** Google
Search Console» и помечает каждого провайдера тегом «минимум для pulse» /
«дополнительно» (данные — поле `tier` в `PROVIDERS`, `yandex` и `google-sa`
помечены `minimum`). Ни одна команда не пропала и не переименована — только
представление; список команд и их скрипты не менялись. Тесты:
`tests/test_cli_dispatch.py` (`test_help_starts_with_today_group`,
`test_every_command_has_a_valid_group` — фиксированный total, не выводимый из
...
**#28** `sed: RE error: illegal byte sequence` — корень не в локали и не в
кириллических комментариях шаблона (проверено в девяти локалях — sed над
шаблоном везде проходит), а в ответе на вопрос 5: backspace над кириллицей в
терминале macOS стирает один байт, `read -r` получает битый UTF-8
(`project_type: и�bд�log` в отчёте), первый sed молча пишет его в
`seo-cycle.yaml`, следующий падает на этой строке и оставляет временник
`.!PID!seo-cycle.yaml`. Теперь `read_answer` проверяет ответ на валидный UTF-8
(битый → предупреждение в stderr + значение по умолчанию), а `sed_in_place` и
единственный прямой `sed` работают под `LC_ALL=C` (побайтно: не-ASCII байт в
файле или в замене никогда не роняет прогон; шаблоны литеральные, без
regex-метасимволов вне ASCII, поэтому для валидного ввода результат побайтно
тот же, что в UTF-8-локали — кириллические литералы вроде `city: "Москва"`
совпадают побайтно).
**#29** `EOFError` в `project-intake-wizard.py` — оболочка читала ответы из
`/dev/tty`, а подробный intake — из `stdin`; при запуске из-под агента/пайпа
первый вопрос падал трассировкой, а init печатал «✓ project profile создан».
Теперь мастер в `--interactive` берёт ответы из `/dev/tty`, когда stdin не
терминал; нет ни того, ни другого — внятная строка в stderr и код 2 (ошибка
среды, как «конфиг не найден»; не 3 — это код «нужно согласие/LLM») без записи
файлов (на `--defaults` молча не переключается: человек просил опрос); `EOF`
и Ctrl-C — тот же честный исход; битый UTF-8 в ответе intake — переспрос,
а не U+FFFD в файле; вступление и вопросы идут в тот же терминал. `init-project.sh` при ненулевом коде мастера
печатает «ℹ project intake не заполнен … — запусти …» вместо «✓». Тесты:
`tests/test_intake_wizard_tty.py` (pty-стенд: stdin=/dev/null, /dev/tty есть —
ровно картина отчётов; init под `LC_ALL=C` и `POSIX` с битым байтом в ответе
5 и «y» на шаге 22 — на прежнем коде rc=1, временник и невалидный UTF-8,
теперь validate rc=0; побайтное сравнение вывода `sed_in_place` в C и UTF-8).

### Fix: топ-500 по показам выдавался за KPI сайта — граница выборки (T-096)

`pulse` снимает из Вебмастера/GSC топ-N запросов **по показам** (N = 500) и
на этой выборке считал «топ-10», дельты и факты KPI, нигде не печатая ни N,
ни число запросов сайта. Выборка меняет состав между срезами, и сырая
арифметика бакетов принимала смену состава за потерю позиций — правило трёх
(gsse.ru «топ-10 −8» 28.08, emwoody.ru нули по выпавшим запросам 28.08,
pifagorlab.com 21.08). Теперь: размер выборки — явный ключ конфига
`monitoring.sample.size` (дефолт 500 = прежнее поведение; Вебмастер получает
`--limit`, GSC — `--row-limit` только при явно заданном ключе, его дефолт
5000 строк query×page не урезается молча); `snapshot-build --source gsc`
помечает срез как `query_sample` с `available_rows: null` (GSC итога не
отдаёт); общий хелпер `seo_cycle_core.monitoring.snapshot_sample()` читает
границу из снапшотов даты, и `position-progress`, `pulse`, `kpi-contract`,
`seo-forecast` печатают одну строку «выборка N из M запросов сайта» (старые
снапшоты без `sample` читаются, M → «M неизвестно»). `position-progress`
добавляет блок `overlap_vs_previous`/`overlap_vs_first`: топ-3/10/30 и
средняя позиция сравниваются только внутри пересечения выборок, заголовочные
дельты берутся из него, сырая разница печатается отдельно с подписью «состав
выборки, не позиции», выпавшие запросы подписаны «выбыл из выборки». Алерт
`top10_drop` в `pulse`, «Δ топ-10» портфеля и карточки топ-3/10/30 веб-дашборда
(`webapp.py`) — только по пересечению;
`progress.json` без блока пересечения (до правки) алерта не даёт. Факты KPI
подписаны «по выборке топ-N». Тесты: `tests/test_sample_boundary.py`
(фикстура «те же позиции, другой состав»: на прежнем коде — «Топ-10 2 (−1↓)»
и critical-алерт, теперь «+0 по пересечению 2 запросов» без алерта;
контроль — настоящая просадка внутри пересечения алерт даёт; печать «N из M»
во всех отчётах; «M неизвестно» на старом снапшоте).
### Fix: остаток денежного класса — согласие и учёт в местах вне сокетного шлюза (T-069, R3-5 гейта T-066 круга 3)

Гейт круга 3 (`optimize/reports/2026-09-06-review-T-066-round3.md`, R3-5)
назвал места, которые тратят деньги или квоту, но лежат за границей
`spend_guard` (внешние CLI, `curl` в shell, клики в браузере) или не имели
предполётки на том же вызове, что сосед. **Ломающее изменение CLI** (§6 —
кандидат на major при выпуске; `VERSION` в этом пакете не поднят, решение за
релизом): четыре точки входа теперь требуют явного согласия, без него ничего
не отправляется и не запускается, код выхода **3** («нужно `--live`»),
код **1** — леджер отказал, **2** — ошибка вызова/конфига.

- **`rag-query.py` ↔ `rag-index.py` на одном платном вызове `/embeddings`.**
  `rag-query` (гибридный режим при настроенном `EMBEDDING_API_*`) делает ту же
  предполётку usage-ledger (`embedding_api`, `llm`, `--requests 1`) до
  открытия базы и пишет расход после вызова. При этом сам «образец»
  `rag-index.py:127` оказался мёртвой веткой: `ledger_preflight` вызывался БЕЗ
  метрики, а `usage-ledger.py check` без метрики отвечает rc=2 («check
  requires at least one estimate metric») — индексация с настроенными
  эмбеддингами блокировалась ВСЕГДА (безопасно, но не работало). Оба вызова
  теперь с `requests=1`. `rag query --global` вне проекта (нет
  `seo-cycle.yaml` в cwd) учесть расход не может: `--mode auto` → BM25 с
  пометкой в stderr, явный `--mode hybrid` → rc 2. `search()` получил
  необязательный out-параметр `stats` (`embedding_calls`) — запись только за
  фактический вызов.
- **`llm-cli-collect.sh`, `img-generate.sh` (claude-режим), `nw-cli.sh new|plagiarism`
  — обязательный `--live`** плюс write-ahead строка в usage-ledger через
  `usage-ledger.py record --fail-on-block` ДО запуска внешней команды
  (`llm_cli`/`llm` `--requests N`; `codex`/`llm` `--requests 1`;
  `neuronwriter`/`paid_api` `--content-writer 1` или `--plagiarism-checks 1`).
  Потолок, битый журнал или отсутствие `seo-cycle.yaml` в cwd — стоп до
  вызова. Свежий кэш `llm-cli-collect` и codex-режим `img-generate`
  (`CODEX_NATIVE_IMAGE`) не тратят и `--live` не требуют; `nw-cli
  projects|queries|get|content|evaluate|import` — чтение, без флага.
  Внешняя команда лежит за границей сокетного шлюза — гейт и запись в
  обёртке единственное, что здесь возможно (см. «Граница защиты» 2.2.0).
- **`writerzen-browser-collect.py` — обязательный `--live`** (обычный запуск
  открывал браузер и создавал отчёты — кредиты тратились вообще без флага;
  `--dry-run` как прежде), предполётка `writerzen`/`paid_api`
  `--requests <отчётов>` до запуска браузера, запись **по факту**: по
  `actions` раннера считаются отчёты с `report_started` (повторное
  использование готового отчёта кредиты не тратит и не считается).
- **`gsc-request-indexing-browser.py` — учёт кликов по факту.** Согласие уже
  было (`--auto-click`; без него `manual_action_required`); добавлена запись
  `gsc_request_indexing`/`browser` `--requests N`, где N — строки результата с
  действием `request*` (сам статус `submitted_or_requested` кликом не
  является — он ставится и для уже отправленного URL). Честная граница
  п. 4 пакета: клик в браузере файловый учёт увидеть не может; write-ahead
  невозможен (число кликов известно только из результата раннера), запись —
  аудит, не стоп (у суточной квоты Google потолка в леджере нет, стоп — её
  собственный `quota_hit`); раннер, убитый между кликом и записью
  result-file, теряет этот клик — названо, не закрыто.
- **Фикс-круг 1 (гейт 2026-09-13, F-1/F-2 — перечень класса рос шестой раз,
  потому что греп исполнителя не был рекурсивным и не обходил вызывающих
  стока).** `page-outline-v3.py --rag` — ТРЕТИЙ вызывающий `rag.search()`
  (по одному платному эмбеддингу на каждую MVP-страницу; команда
  `--all-mvp --rag --write` рекомендована в `rag-usage.md` и `seo-writing`) —
  та же предполётка (`embedding_api`, `--requests <страниц>`) до первого
  вызова и запись по числу реально сделанных; блок → rc 2, без
  `seo-cycle.yaml` — BM25. `scripts/knowledge/graphify-refresh.sh`
  (README/INSTALL/GUIDE/`project-upgrade-assistant`) — живая health-проба
  `agy`/`gemini`, до 35 файлов через LLM CLI или `graphify extract` по
  API-ключам из env — без флага и учёта; теперь обязательный `--live`
  (или `GRAPHIFY_LIVE=1`) при любом достижимом LLM-пути (CLI или ключ),
  иначе код 3 до первой пробы; write-ahead `graphify`/`llm` `--requests 1`
  `--fail-on-block`; сборка корпуса и локальный граф без LLM — бесплатны и
  флага не требуют. Тесты (+8): красные на прежней редакции обоих файлов,
  6 мутаций с якорем убиты.
- **`usage_ledger.finite_nonneg` (R5-1):** 400-значное целое в файле учёта
  роняло саму проверку (`math.isfinite` → `OverflowError`) вместо ответа
  «непригодно»; теперь `False`.
- Тесты `tests/test_t069_paid_remainder.py` (32 после фикс-круга 1): каждая проба гоняет
  настоящий скрипт подпроцессом и смотрит на внешнюю границу — локальный
  HTTP-слушатель `/embeddings` со счётчиком, подставные `codex`/`agy`/`curl`/
  `node` первыми в PATH с журналом вызовов; доказательство учёта — строка
  JSONL, доказательство отказа — пустой журнал + код выхода; негативные
  контроли по потолкам. Мутации по 16 исполняемым строкам — все убиты.
- Не тронуто, с обоснованием (см. «Результат» пакета T-069): `psi-fetch.py`
  (бесплатный сервис, суточная IP-квота, самоограничение 429 — денег нет,
  месячный леджер к суточной квоте не применим), `serpstat-audit.py`/
  `xmlriver-source-pack.py`/`atp-fetch.py` (гейт + `bump_counter` write-ahead
  + `armed_spend` уже есть с T-092; полный USD-учёт невозможен — цена до
  вызова неизвестна), решение R5-2 о поведении force-флага — за Vladimir.
...
`install.sh`, `ensure_worktree()`: сравнение локального пина с origin
запрашивало у сервера ТОЛЬКО точную ссылку тега (`ls-remote --tags origin
"refs/tags/$tag"`) — для аннотированного тега git в этом случае не отдаёт
разыменованную строку `^{}` (коммит), а только сам объект тега. Локальная
сторона (`rev-parse "$tag^{commit}"`) брала именно коммит, поэтому для
любого аннотированного тега (а это ВСЕ релизные теги проекта, `git tag -a`)
сравнение расходилось всегда — `install.sh --upgrade-all --pin` отказывал
на всех проектах сразу после публикации тега, хотя хранилище было синхронно
с origin. Найдено при настоящем перепине четырёх живых проектов на v2.2.0;
защита сработала консервативно (отказ, а не тихое согласие) и портфель не
пострадал. Запрос к origin теперь явно просит обе ссылки
(`"refs/tags/$tag" "refs/tags/$tag^{}"`), что для аннотированного тега
возвращает и объект, и разыменованный коммит, а для лёгкого — не меняет
результат (объект уже и есть коммит). Тестовые стенды сверки тегов
(`tests/test_installer_contracts.py`) до этой правки строили только лёгкие
тестовые теги, где дефект не проявлялся, — добавлен класс
`AnnotatedTagComparisonTest` с матрицей «форма тега (аннотированный/лёгкий)
× состояние (совпадает/расходится/отсутствует)» и негативный контроль,
краснеющий при возврате дефекта.

## [2.2.0] — 2026-09-07

### Fix: отметка времени в локе проекта — микросекунды вместо секунд

`install.sh`, `write_lock_entry()` штамповал поле `updated` в
`.agents/external-skills.lock.yaml` с точностью до секунды. Два перепина,
случившиеся в одну календарную секунду, давали побайтово одинаковый лок — момент
реальной перезаписи в логе не фиксировался. Побочно это делало нестабильным
сторожевой тест защиты установщика: на быстром раннере обе записи попадали в одну
секунду (замерено: 189 коллизий из 200), и проверка «лок обязан измениться»
краснела при честно снятой защите. Точность повышена до микросекунд; после правки
— 0 коллизий из 200, целевой класс прогнан 32 раза подряд без падений. Формат поля
нигде не разбирается и не сравнивается точно, старые локи с секундной отметкой
читаются без изменений.

### Fix: денежный шлюз опущен на уровень сокета + граница защиты названа прямо (T-089 круг 3, R2-1/R2-2/R2-3/R2-5/R2-9)

Третий независимый гейт вернул круг 2: заявление «проверка на транспорте, не
на клиенте» не выдержало проверки боем — `urllib.request.urlopen`/`requests.
Session.request` подменялись на месте, но существовали только в процессе,
который их импортировал, и обходились семью способами внутри процесса
(ранний импорт имени, `build_opener`/редиректы, `http.client` напрямую,
`socket` напрямую, хост в байтах, хост с точкой на конце, прямой вызов
сохранённого оригинала). Периметр не стал структурным — переехал со
«списка декорированных функций» на «список файлов, импортирующих
`spend_guard`». Отчёт: `optimize/reports/2026-09-07-review-T-089-round2.md`.

- **`seo_cycle_core/spend_guard.py` переписан на сокетный уровень.**
  Патчатся `socket.getaddrinfo` (резолвинг — через него идут `urlopen`,
  `build_opener`, `http.client`, `requests`/`urllib3`) и
  `socket.socket.connect`/`connect_ex` (сырые сокеты, которые резолвят имя
  на уровне C, минуя Python-функцию `getaddrinfo`). Оба патча закрывают шесть
  из семи обходов круга 2 в одном месте вместо одного патча на библиотеку.
  Седьмой — прямой вызов сохранённого оригинала (и перезагрузка модуля
  `socket`) — в Python неустраним в принципе; он честно назван в разделе
  «Граница защиты» как то, что ловится ревью кода, а не механизмом.
  Разбор хоста почищен: байты декодируются, точка на конце обрезается,
  регистр приводится — один раз, в одной точке для всех вызовов.
- **`seo_cycle_core/__init__.py`** теперь импортирует `spend_guard` как
  побочный эффект импорта пакета — активация больше не зависит от того,
  вспомнил ли автор нового клиента про `spend_guard` явно: она наступает,
  как только файл импортирует хоть что-то из `seo_cycle_core` (это делают
  все клиенты, реально вызывающие `armed_spend()` — T-092: сверено
  командой `grep -rl 'armed_spend(' scripts/*.py | grep -v seo_cycle_core
  | wc -l` → 13 на момент этой правки, не «восемь»: изначальный список
  T-089 (ads-apply, competitor-discovery, dataforseo-fetch,
  google-ads-fetch, google-nlp-audit, keyso-fetch, keyso-save, spyfu-fetch,
  yandex-direct-fetch — уже 9, не 8) плюс четыре клиента, добавленные T-092
  (atp-fetch, serpstat-audit, serpstat-fetch, xmlriver-source-pack) —
  число растёт с каждым новым клиентом и не хранится в прозе, а
  пересчитывается той же командой).
- **`tests/test_t089_closed_world_hosts.py`** — новый тест: каждый файл,
  упоминающий хост из `PAID_HOSTS`, обязан импортировать `seo_cycle_core` И
  вызывать `armed_spend(` — не только классифицировать хост (круг 2 этого
  не требовал, R2-3(а)). Проверкой этого теста найден **третий**, никем не
  названный клиент `api.keys.so` — `keyso-save.py`: писал счётчик расхода
  ПОСЛЕ вызова, тот же класс, что F-1. Закрыт тем же механизмом.
- **`tests/helpers/t089_signal_target.py`** — харнесс для `ads-apply.py`/
  `yandex-direct-fetch.py`/`google-ads-fetch.py` теперь вызывает настоящий
  `main()` этих скриптов (R2-9: раньше харнесс сам собирал `armed_spend`/
  `ledger_record()`, продуктовый код в части ячеек матрицы «клиент × сигнал»
  не исполнялся вовсе — проверял харнесс, не продукт). Матрица —
  `tests/test_t089_writeahead_matrix.py:CLIENTS × SIGNALS` = 8 клиентов ×
  4 сигнала = **32 ячейки** (T-092: сверено командой, было ошибочно
  написано «24» — то было 6×4 из предыдущего круга ревью, число не
  пересчитали при добавлении клиентов; девятый клиент по write-ahead,
  `keyso-save.py`, в эту сигнальную матрицу не входит — закрыт тем же
  механизмом `armed_spend()`, но её `CLIENTS` список не расширялся).
- **Граница защиты — прямым текстом, не переоценивая** (полный текст — в
  докстринге `spend_guard.py` и в «Результате» пакета T-089): шлюз не
  защищает (1) процесс, ни разу не импортировавший `seo_cycle_core` —
  компенсация статическая (тест выше), не рантайм; (2) отдельный процесс/
  shell-скрипт (`nw-cli.sh` уже дёргает `curl` напрямую) — вне области
  видимости обеих половин; (3) сырой сокет на уже резолвленный числовой
  IP — сравнивать не с чем; (4) браузер, внешний бинарник, другой язык.
  Компенсация для (1)/(2) — код-ревью и статический скан, не код.

### Fix: денежный стоп переведён с per-client декоратора на единый транспортный шлюз (T-089, F-1/A/C/D/H)

Второй независимый враждебный QA перед тегом 2.2.0
(`optimize/reports/2026-09-07-seo-cycle-qa-v220.md`, F-1) нашёл, что
`google-nlp-audit.py` — единственный из шести платных клиентов, оставшийся
на конструкции «учёт расхода ПОСЛЕ платного вызова» (класс, который T-066
«Круг 4» явно назвал негодным для остальных пяти): SIGINT/SIGTERM/SIGKILL
между отправкой запроса и записью терял расход целиком, без второго рубежа.

Круг 1 закрыл это через `@guarded_spend`-декоратор на функции каждого
клиента. **Независимый второй гейт вернул пакет**
(`optimize/reports/2026-09-07-review-T-089.md`): декоратор — дисциплина
код-ревью, а не структурный барьер. `functools.wraps` оставляет исходную
функцию доступной как `fn.__wrapped__` (finding A), а новый платный клиент,
который декоратор просто не применил, никем не замечен (finding C) — и это
было не гипотезой: `keyso-fetch.py`/`competitor-discovery.py` уже были в
дереве, уже били в платный `api.keys.so`, уже писали счётчик ПОСЛЕ вызова
(finding H).

- **`google-nlp-audit.py`** переведён на write-ahead: `add_usage()`/
  `save_usage()` теперь стоят ДО `call_feature()`, а не после — юнит-цена
  здесь известна заранее (по длине текста), поэтому, в отличие от
  DataForSEO/SpyFu, отдельного состояния «сумма неизвестна» не нужно.
- **`seo_cycle_core/spend_guard.py`** переписан: точка отказа перенесена с
  декоратора клиентской функции на сам транспорт — `urllib.request.urlopen`
  и `requests.sessions.Session.request` патчатся один раз, на месте, при
  импорте модуля. `PAID_HOSTS` — реестр хостов, звонок на которые без
  активного `armed_spend(write_ahead, hosts=...)` для этого хоста
  поднимает `SpendNotArmedError` ДО отправки байта в сеть. Обёртки вокруг
  клиентских функций больше нет — снимать нечего.
- **`keyso-fetch.py`/`competitor-discovery.py`** закрыты тем же механизмом:
  `api.keys.so` в `PAID_HOSTS`, счётчик перенесён на запись ДО вызова.
- **`tests/test_t089_closed_world_hosts.py`** (новый) — статический гейт:
  сканирует все `http(s)://`-литералы в `scripts/*.py` и падает по имени
  файла и хоста, если найденный хост не классифицирован ни в `PAID_HOSTS`,
  ни в `FREE_HOSTS`. Закрывает половину finding C, которую транспортный
  шлюз структурно закрыть не может (см. ниже).
- **Честная граница механизма** (не переоценка, как в круге 1): транспортный
  шлюз гарантирует, что звонок на УЖЕ ЗАРЕГИСТРИРОВАННЫЙ платный хост без
  write-ahead невозможен структурно — ни через `__wrapped__` (обёртки нет),
  ни через прямой вызов клиентской функции, ни через копипаст в новый файл,
  ни через полностью незнакомый со `spend_guard` восьмой/девятый клиент,
  если он бьёт в уже известный хост. Он НЕ может знать, что хост из
  СОВСЕМ НОВОГО провайдера — платный, до того, как кто-то его туда впишет
  — эту половину закрывает статический тест выше (падает на этапе
  ревью/CI, а не в рантайме). `armed_spend()` также доверяет переданному
  `write_ahead`, что тот реально написал на диск, а не соврал — это
  осталось конвенцией для тестов (`lambda: True`), не может быть
  верифицировано структурно без повторного чтения файла с диска (что и
  делает каждый реальный `write_ahead` в продукте). Арминг привязан к
  набору хостов, а не к одному вызову — умышленно: `ads-apply.py`
  легитимно делает одну запись на целую пачку операций (T-066 R3-4),
  разовый арминг сломал бы этот паттерн; что арминг больше не течёт
  МЕЖДУ хостами (round-1 finding D) — реальное исправление.
...
- **F-13, ближе к настоящему (но всё ещё не до конца — см. круг 3 ниже).**
  `call()` (dataforseo, spyfu) больше не завершает процесс — поднимает
  `ApiCallError` (новый класс в общем модуле). `fetch()`/`run()` ловит его под
  той же блокировкой, пишет учёт, и только потом `sys.exit`. Тесты — на
  уровень ниже: мокается `urllib.request.urlopen`. Круг 2 ловил в `call()`
  ПЕРЕЧЕНЬ конкретных типов исключений (`HTTPError`/`URLError`/`TimeoutError`)
  — обрыв тела уже отправленного запроса другим типом (`IncompleteRead`,
  `ConnectionResetError`, `ssl.SSLError`, `MemoryError`) пролетал мимо и мимо
  записи расхода; это нашёл третий гейт, не второй.
...
- **F-13 в двух потребителях usage-ledger.py** — `yandex-direct-fetch.py` и
  ...
  — квота уже потрачена, учёт не узнавал. `ads-apply.py` был заявлен как
  «проверен отдельно, уже безопасен» — **это заявление оказалось неверным**,
  проверкой не подтверждено (первый гейт не гонял эту ветку вовсе); настоящая
  проверка и фикс — в круге 3 ниже.
...
- **Обзор денежных клиентов с файловым учётом (не полный обзор класса —
  см. круг 4)** — `serpstat-fetch.py --min-credits` (живой
...
### Круг 3 (независимый гейт вернул пакет в `todo` во второй раз): write-ahead вместо перечня, шестой денежный стоп, полнота класса

Круг 2 чинил по перечислению (адреса → сценарии → типы исключений) — третий
гейт снова нашёл следующий пункт: **перечень типов исключений в `call()`**
(`HTTPError`/`URLError`/`TimeoutError`) не покрывает обрыв тела УЖЕ
отправленного запроса другим типом (`IncompleteRead`, `ConnectionResetError`,
`ssl.SSLError`, `MemoryError`) — такое исключение пролетало мимо
`ApiCallError` и мимо записи расхода, голым traceback'ом, ровно как в круге
1. Компромисс, названный прямо: часть фиксов ниже **over-records** — считает
попытку потраченной ДО того, как известен её реальный исход, потому что цена
недосчитанного платного вызова выше цены лишней строки в леджере.

- **R2-1, шестой денежный стоп.** `scripts/usage-ledger.py:read_ledger_events()`
  не проверял строки САМОГО журнала `usage-ledger.jsonl` на пригодность
  значения вообще — одна строка с отрицательным `usd` вычиталась из месячного
  расхода и переводила governance-гейт (`ledger_preflight()`, который решает,
  пускать ли `ads-apply.py`/`google-ads-fetch.py`/`yandex-direct-fetch.py`) из
  `blocked` в `ok`. Легального канала «возврат/коррекция» в этом файле нет
  (`grep -niE "refund|adjust|correction|возврат|коррекц"` — пусто), поэтому
  отрицательное значение сегодня не имеет ни одного законного смысла. Теперь
  каждая строка проверяется: `metrics` обязан быть словарём, каждое значение —
  `finite_nonneg`, `month` — по маске; иначе строка становится `_error` и
  выпадает из суммы (`evaluate()` уже умел превращать `_error` в `blocked`).
  **Цена, названная честно**: месяц, в истории которого уже лежит отрицательная
  строка от версии до этого фикса, будет заблокирован до РУЧНОЙ чистки
  журнала — автомиграции нет и не будет (fail-closed). Найти такие строки:
  `grep -n '"usd": *-' seo/usage/usage-ledger.jsonl` (и то же для `rows`/
  `requests`/`units`/`credits` в поле `metrics`).
- **R2-2, инверсия вместо перечня, во всех шести клиентах.**
  `dataforseo-fetch.py`, `spyfu-fetch.py` — `except Exception` вокруг всего,
  что идёт после `urlopen`/`read()`, вместо перечня типов.
  `yandex-direct-fetch.py`, `google-ads-fetch.py` — write-ahead:
  `ledger_record(requests=1, ...)` пишется ДО `live_fetch()`/`gaql_search()`
  (пессимистично — квота считается уже потраченной фактом отправки), при явной
  неудаче — уточняющая запись; любое исключение/сигнал после первой строки уже
  не теряет факт попытки. `ads-apply.py` — заявление предыдущего круга «уже
  безопасен» проверкой не подтвердилось: `apply_direct()` ловил перечень типов
  (`URLError`/`KeyError`/`JSONDecodeError`) для КАЖДОЙ операции цикла;
  `ConnectionResetError`/`TimeoutError`/`ValueError` уходили из функции
  ЦЕЛИКОМ — операции ПОСЛЕ упавшей вообще не выполнялись и не попадали в
  `results`. Теперь `except Exception` на каждой операции, `ledger_record()`
  после цикла достигается всегда. `google-nlp-audit.py` — тот же класс на два
  фронта: `except requests.exceptions.RequestException` расширен до
  `except Exception`, и порядок строк (запись кэша ответа на диск стояла
  ПЕРЕД учётом расхода — сбой записи по `OSError` терял расход) исправлен на
  учёт-сначала-кэш-потом.
- **R2-3.** «Сумма неизвестна» — не «сумма ноль»: `dataforseo-fetch.py`
  считало `cost_unknown_calls`, но `spent_usd` этим не двигался, и денежный
  стоп не срабатывал ни разу при массовом сбое поля `cost` в ответе (200
  вызовов проходили при `--budget 1`). Теперь `cost_unknown_calls > 0` без
  `--force` блокирует дальнейшие платные вызовы. У `spyfu-fetch.py` этот класс
  не применим — там `cost` считается детерминированно из `rows * cpm`, а не
  читается из ответа API.
- **R2-6.** `test_serpstat_min_credits.py` вызывал `nonneg_int_arg(...)`
  напрямую — отцепление валидатора от флага (`type=int` вместо
  `type=nonneg_int_arg` в реальном парсере) оставляло сюиту зелёной.
  Переписан через `build_parser().parse_args([...])` (парсер вынесен из
  `main()` тем же паттерном, что у dataforseo/spyfu).
- **R2-5.** `test_google_nlp_ledger.py` (13 тестов) не выполнялся в CI
  вовсе — джобы `unittest`/`coverage-core` ставили только `.[dev]`, без
  опциональных зависимостей `google-nlp-audit.py` (`requests`,
  `beautifulsoup4`, `google-auth`). Точечно добавлены в обе джобы (не весь
  extras `google`).
- **R2-4, полнота класса.** `competitor-discovery.py` не был заявлен в
  таблице класса, хотя тратит ту же квоту `api.keys.so`, что и
  `keyso-fetch.py`/`keyso-save.py`: `--ttl` был голым `type=float` (тот же
  дефект R-4, уже закрытый у трёх других клиентов) — исправлено через
  `nonneg_finite_arg`. Счётчик квоты Keys.so считал только `keyso-fetch.py` —
  вынесен общий `bump_counter()` (`seo_cycle_core/usage_ledger.py`), все три
  клиента (`keyso-fetch.py`, `competitor-discovery.py`, `keyso-save.py`)
  теперь пишут в один `_usage.json`.
  **Открытые вопросы, зафиксированы честно, не починены в этом релизе:**
  - `atp-fetch.py` (AnswerThePublic, ~8 кредитов/запрос по его же docstring) —
    ни стопа, ни учёта; в `config/project.template.yaml` нет ни одного ключа
    конфигурации для ATP вообще (`grep -n atp config/project.template.yaml` —
    пусто). Предложение: секция `governance.subscriptions.answerthepublic.
    monthly_credit_cap` + локальный `_usage.json` под
    `seo/research/answerthepublic/`, тот же паттерн, что у keyso.
  - `serpstat-audit.py` — платный вызов только за флагом `--live`, но своего
    учёта расхода кредитов нет (в отличие от `serpstat-fetch.py`, у которого
    есть live-preflight через `--min-credits`). Предложение: тот же
    `--min-credits`-preflight на `getStats`, что уже есть у `serpstat-fetch.py`
    — переиспользовать, а не изобретать заново.
  - `xmlriver-source-pack.py` — деньги за строку захардкожены в коде
    (`PRICE_RUB_PER_1000`), за флагами `--live --allow-paid`, но своего
    файла учёта и денежного стопа нет. Предложение: `_usage.json` +
    `governance.subscriptions.xmlriver.monthly_rub_cap` (рубли, не доллары —
    прайс уже в рублях), тот же общий модуль.
  - `seo_cycle_core/rag.py:embed_texts` — вызов `/embeddings` по
    `EMBEDDING_API_URL`; платный ТОЛЬКО если переменная указывает на платного
    провайдера (может быть локальная Ollama) — ни в коде, ни в докладной
    документации нет гарантии, какой случай сейчас настроен у конкретного
    проекта. Предложение: если у переменной есть хотя бы один живой
    настроенный платный провайдер в проекте — тот же паттерн стопа/учёта;
    если только локальные — явно задокументировать в `docs/rag.md`, что
    денежного класса тут нет, вместо молчания.
  Ничего из этих четырёх пунктов не тронуто кодом — только зафиксировано
  здесь и в «Результате» тикета T-066 с конкретным предложением по каждому,
  как требовал третий гейт.

### Круг 4 (третий независимый гейт вернул пакет в `todo` в третий раз): write-ahead на правильном уровне, а не на уровне исключения

Круг 3 закрыл потерю расхода конструкцией `except Exception` в `call()`
(dataforseo, spyfu) — третий гейт нашёл следующий пункт того же класса:
`except Exception` **не ловит** `BaseException` (`KeyboardInterrupt`/Ctrl-C,
`SystemExit`, `GeneratorExit`), и ни один `except` вообще не ловит сигнал
(`SIGTERM`/`SIGKILL`). Инверсия «перечень типов → `Exception`» решала не ту
задачу: правильный уровень — не перехват ПОСЛЕ отправки запроса, а запись
намерения потратить (write-ahead) ДО неё.

- **R3-1, write-ahead на правильном уровне (`dataforseo-fetch.py`,
  `spyfu-fetch.py`, `ads-apply.py`).** `fetch()`/`run()` теперь инкрементируют
  `calls`/`cost_unknown_calls` и вызывают `save_usage()` ДО `call()`, а не
  полагаются на перехват исключения после него; при успешном ответе с
  известной суммой pending-запись уточняется на месте (`cost_unknown_calls`
  −1, `spent_usd` +cost) — это НЕ отдельная «уточняющая» запись, а правка той
  же строки. Ctrl-C/`SystemExit`/`SIGKILL` в любой точке после write-ahead уже
  не теряют факт платного вызова: файл на диске говорит `cost_unknown_calls`
  ≥ 1, и R2-3 (уже построенный в круге 3) блокирует дальнейшие платные вызовы
  до `--force`. Для `SIGKILL` это не «поймали сигнал» (его не ловит ничто) —
  это «запись уже лежала на диске ДО того, как процесс убили», что и есть
  единственный переживающий такое обрыв механизм. `spyfu-fetch.py` не знает
  cost/rows до ответа API — поэтому получил свой `cost_unknown_calls` и
  свой блокирующий гейт по аналогии с R2-3 (раньше там не было ни такого
  поля, ни стопа на нём). `ads-apply.py` — `ledger_record()` для всей пачки
  операций (`requests=len(operations)`) теперь стоит ДО цикла `apply_direct()`,
  а не после: круг 3 закрыл потерю расхода ВНУТРИ цикла (перехват на каждой
  операции), но сама запись расхода на пачку осталась после цикла целиком —
  обрыв посреди цикла терял ВСЕ уже выполненные и оплаченные операции, не
  только последнюю.
- **R3-3, возврат `ledger_record()` теперь проверяется на write-ahead
  вызовах.** Круг 2 переделал `ledger_record()` (`seo_cycle_core/ads.py`) в
  `bool` ради видимости отказа записи — но ни один из трёх вызывающих
  (`yandex-direct-fetch.py`, `google-ads-fetch.py`, `ads-apply.py`) возврат не
  проверял: платный вызов уходил в сеть, даже если журнал физически не принял
  запись (например каталог только на чтение), и процесс выходил кодом 0 с
  пустым журналом. Теперь `False` останавливает выполнение ДО платного
  вызова (`sys.exit`-эквивалент, ненулевой код), а не только печатает
  предупреждение постфактум.
- **R3-4, честность вместо несуществующего механизма.** Круг 3 обещал в
  коде и здесь «уточняющую запись `requests=0` при неудаче» для
  `yandex-direct-fetch.py`/`google-ads-fetch.py` — она не срабатывала НИ
  РАЗУ: `usage-ledger.py record` отказывает без единой ненулевой метрики
  (`requests=0` и `usd=0` — обе «нет метрики»), поэтому вторая запись
  постоянно проваливалась тем же кодом, что случайный сбой. Убрано: теперь
  ровно одна запись (write-ahead), при неудаче самого вызова — только
  предупреждение в stderr, без второй (несуществующей) записи в журнал.
- **R3-2, `atp-fetch.py` — единственное место в репозитории без гейта, учёта
  и стопа одновременно.** Обычный запуск (`python3 atp-fetch.py "фраза"`)
  списывал ~8 кредитов AnswerThePublic сразу, без единого предохранителя.
  Это НЕ полный учёт (полноценный `_usage.json` + `governance` лимит —
  отдельный пакет, см. ниже) — только обязательный `--live` перед
  `create_search()`, по образцу `xmlriver-source-pack.py`: без флага скрипт
  отказывает ДО сети. `--me` (health check) и `--report-id` (pull уже
  созданного отчёта) бесплатны и `--live` не требуют.
- **Мост `ads.ledger_record()` → подпроцесс → JSONL, впервые проверен
  реальным вызовом.** До этого круга ни один тест не проверял цепочку
  `ledger_record() → subprocess usage-ledger.py record → строка в
  usage-ledger.jsonl` иначе как через `mock.patch.object(..., "ledger_record")`
  — то есть проверялся порядок вызова заглушки, а не факт записи и не
  возвращаемое значение. Новый `tests/test_ads_ledger_bridge.py` вызывает
  настоящую функцию с временным `project_root`: проверяет, что строка реально
  появляется в JSONL (успех) и что возврат — `False`, когда каталог
  недоступен на запись (сбой). Это и есть первопричина того, что R3-3
  пережил два предыдущих гейта незамеченным.
- **Честно про заголовок класса (R3-5).** «Полный обзор класса» в круге 2 —
  неточная формулировка: расширенный признак (сессии/`http.client`/`httpx`,
  внешние CLI через `subprocess`, `curl` в `*.sh`, playwright-раннеры) даёт ещё
  минимум семь мест, тратящих деньги/квоту мимо файлового учёта этого класса:
  генерация изображений (`scripts/img-generate.sh`), квота подписки
  NeuronWriter (`scripts/nw-cli.sh`), токены Gemini/Codex
  (`scripts/llm-cli-collect.sh`), квота PageSpeed Insights (`scripts/psi-fetch.py`),
  два playwright-раннера, тратящие квоту кликами мимо любого леджера
  (`gsc-request-indexing-runner.mjs`, `writerzen-browser-runner.mjs`), и
  асимметрия `rag-index.py` (делает `ledger_preflight`) против `rag-query.py`
  (не делает вовсе на том же платном вызове) — самая близкая к настоящей дыре
  из семи, первая строка следующего пакета. Ни одно из этих семи мест этим
  тикетом не тронуто — список зафиксирован здесь и в «Результате» тикета
  T-066 для отдельного пакета после тега.

### Круг 5 (четвёртый независимый гейт вернул пакет в `todo` в четвёртый раз): значение из файла учёта проверяется по типу, а не по имени из списка

Круг 4, закрывая потерю расхода write-ahead'ом, ввёл новый стоп
(`cost_unknown_calls`) — и повторил на нём ровно ту ошибку, из-за которой
этот тикет идёт на пять кругов: `usage_ledger.load_usage()` проверяла
значение только у полей, перечисленных по имени в `USAGE_FIELDS`
(`("spent_usd", "calls")` у dataforseo, `("spent_usd", "rows")` у spyfu) —
новый стоп в этот список никто не вписал, и NaN/Infinity/отрицательное
значение читалось из файла без единой проверки, у обоих денежных клиентов, и
**навсегда** (арифметика `+1`/`-1` возвращает отравленное значение обратно в
файл, `> 0` больше не срабатывает никогда). Это F-11 в шестой раз — на своей
же новой строке, внесённой в круге 4.

- **R4-1, структурный фикс, не точечный.** `load_usage()` больше не
  полагается на список: КАЖДОЕ числовое поле файла (JSON-значение типа
  `int`/`float`, не `bool`) проверяется `finite_nonneg` по признаку своего
  типа — списком `numeric_fields` теперь объявляются только поля, для
  которых вдобавок проверяется, что они вообще ЕСТЬ и являются числом
  (нужно вызывающей стороне для нетипизированной арифметики вида
  `u["calls"] + 1`); любое другое числовое поле, даже никем не
  перечисленное, всё равно не может пройти отравленным. Седьмой стоп,
  который кто-то добавит без этого списка, защищён этим же кодом
  автоматически.
- **R4-2, тест на каждого клиента отдельно.** Дыра в `spyfu-fetch.py` жила
  именно потому, что покрытие было только у `dataforseo-fetch.py` (мутация
  «снять стоп cost_unknown_calls» у spyfu не красила ни одного из 832
  тестов). Добавлены `PoisonedCostUnknownCallsTest` в
  `tests/test_dataforseo_fetch.py` и `tests/test_spyfu_fetch.py` — по
  клиенту, включая позитивный контроль (легитимные значения 0 и 1 не ломают
  сам стоп) и `failed_calls` у spyfu.
- **R4-4.** `GUIDE.md` (обе языковые таблицы, строки про `atp-fetch.py`)
  учил команде без `--live` — она отказывает с круга 4. Добавлен флаг в
  обеих таблицах.
- **R4-3, НЕ чинил — решение о поведении, не однострочная правка.**
  Восстановление после потери сети (`ConnectionRefusedError`, запрос
  физически не ушёл) ставит `cost_unknown_calls=1` тем же write-ahead'ом,
  что и настоящий обрыв дорогого запроса — единственный выход `--force`, а
  `--force` у обоих денежных клиентов одним флагом снимает три разные вещи
  одновременно: отказ по битому файлу учёта, стоп `cost_unknown_calls` И
  месячный денежный лимит. То есть частая безобидная ситуация (пропала
  сеть/VPN/опечатка в хосте) учит человека набирать флаг, отключающий
  денежный стоп заодно. Не тронуто кодом сознательно — см. «Результат»
  тикета T-066 для точного описания поведения и вариантов решения.

- **T-090**: чтение YAML-конфига мимо `seo_cycle_core.config` теперь
  структурно невозможно, а не только нежелательно. Раньше 32 из ~97 файлов
  в `scripts/` читали YAML своим собственным `yaml.safe_load(...)` в обход
  общих гарантий ядра (координата ошибки вместо traceback, отказ на
  не-UTF-8, отказ на не-словаре) — F-8. Плюс два конкретных бага в самом
  ядре: `project: null` в конфиге давал зелёный `✓` у `monthly-dashboard.py`
  и `db-sync.py` (F-7 — `config_section()`/`require_config()` отвергали
  только пустой словарь, а не словарь с `None` внутри нужного ключа), и
  существующий-но-пустой/только-комментарии/`---\n null\n` файл был
  неотличим от «файла нет» у 58 команд (F-7b).
  - `seo_cycle_core/config.py`: рантайм-сторож оборачивает конструкторы
    всех классов PyYAML Loader (`SafeLoader`/`Loader`/`FullLoader`/
    `UnsafeLoader`/`BaseLoader` и C-варианты) — любой код внутри
    `scripts/`, кроме самого `config.py`, при попытке напрямую собрать
    Loader (включая `yaml.safe_load`, который просто конструирует
    `SafeLoader` внутри) падает с понятным `RuntimeError` вместо тихого
    парсинга. Код вне `scripts/` (тесты, сторонние инструменты) не
    затронут — сторож проверяет, что вызывающий кадр лежит именно в
    защищаемом дереве.
  - Новые функции ядра: `load_config()` (алиас на прежний `load_yaml()`,
    строгий разбор конфига проекта), `load_yaml_any()` (терпимый разбор
    ЛЮБОГО YAML — policy/entities/manifest/triggers/region-profiles — без
    принуждения к словарю), `parse_yaml_text()` (для frontmatter-кусков в
    памяти, не файлов), `require_section()` (F-7: раздел обязателен и не
    может быть `null`/пустым/не-словарём — используется в
    `monthly-dashboard.py`/`db-sync.py` для `project`). `config_section()`
    чинит F-7: ключ отсутствует → тихий `{}` (как раньше), ключ есть, но
    не словарь (включая `None`) → WARNING в stderr (раньше `None` тихо
    обходил предупреждение).
  - Все 32 файла (31 `.py` + `monthly-runner.sh`) переведены на вызовы
    ядра точечно (замена вызова загрузки, без переписывания логики);
    `keyso-save.py`'s `except ImportError: return {}` — отдельный опасный
    класс (сломанный PyYAML выглядел как «конфига нет») — тоже закрыт.
    Заодно найдены и починены два новых traceback того же F-7 класса при
    миграции (`governance-report.py`/`growth-roadmap.py`: `cfg.get("project",
    {})` пропускал `None` тем же путём, что и `config_section()` до фикса).
  - Границы, оставленные явно (не строгий `load_config`, а терпимый
    `load_yaml_any`/собственная обработка): `validate-config.py` (его
    работа — диагностировать в том числе битый конфиг, не падать на нём),
    setup/onboarding/intake-визарды (легитимно работают без конфига),
    policy/entities/manifest/triggers/region-profile файлы (не обязаны
    быть словарём верхнего уровня).
  - Новый структурный AST-тест `tests/test_no_yaml_bypass.py` проходит по
    каждому `scripts/**/*.py` и по heredoc-python в `scripts/*.sh`, ищет
    прямой доступ к Loader-классам PyYAML вне `seo_cycle_core/config.py`,
    и доказывает, что реально ловит новый обход (плантит временный
    файл-бypass, проверяет и статическую, и рантайм-поимку, удаляет файл).
  - Байт-сравнение 14 команд на здоровом конфиге (шаблон
    `config/project.template.yaml`) между `origin/main` и веткой —
    0 расхождений (только замаскированные timestamp/путь-к-рабочей-папке).
    Юнит-тестов было 842, стало 861 (+19: 4 в `test_no_yaml_bypass.py`, 15
    в новом `test_config_t090.py`) — все зелёные.

  **Круг 2 (независимый гейт 2026-09-07, `optimize/reports/2026-09-07-review-T-090.md`,
  вернул пакет в `in_progress`).** Гейт нашёл: (1) `load_config = load_yaml` был
  просто алиасом — существующий-но-пустой файл до сих пор давал зелёный `✓` у
  11 из 17 команд без ключей (F-7b по факту не закрыт, обязательная проверка
  расширена с 13 до 13 файлов — 0 роста); (2) три новых traceback того же F-7
  класса (`launch-plan.py`/`tool-stack-recommender.py`/`task-router.py` на
  `project: null`), сплошного прохода по классу не было; (3) мутация решающей
  строки в двух мигрированных файлах (`require_section(...)` → `cfg.get(...)`)
  прошла все 861 тестов зелёными — миграция 32 файлов не была покрыта
  интеграционными тестами; (4) сторож обходится тремя способами (новый файл без
  импорта ядра; `config._GUARD_ENABLED = False` одной строкой;
  `importlib.import_module("yaml")` в комбинации с (б)) — формулировка
  «структурно невозможно» была неточной.

  - **F-7b, читающая функция.** `load_config` — больше не алиас: это отдельная
    строгая функция (`seo_cycle_core/config.py`), которая на файле, который
    СУЩЕСТВУЕТ, но парсится в `None`/`{}` (0 байт, только комментарии, `---\n
    null\n`, `{}`), печатает `ERROR: ... конфиг пуст ... ` и `exit(2)` — раньше,
    на уровне самого чтения, для КАЖДОГО вызывающего `load_config`/локальный
    `load_yaml()`-враппер поверх него, а не только для файлов, отдельно
    зовущих `require_config`. `load_yaml()` остаётся терпимым (та же семантика,
    что раньше) для легитимно-опциональных читателей (policy-оверлеи,
    мультипроектные сканы) — они по-прежнему возвращают `{}` на отсутствующем
    ИЛИ пустом файле, это не регрессия.
  - **Обязательная проверка расширена с 13 до 24 файлов**: `automation-plan.py`,
    `governance-report.py`, `growth-roadmap.py`, `launch-plan.py`,
    `task-router.py`, `tool-stack-recommender.py`, `access-key-assistant.py`,
    `project-upgrade-assistant.py`, `setup-answer-plan.py`, `setup-blueprint.py`,
    `setup-control-plane.py` теперь зовут `require_section(cfg, "project", ...)`
    на пути, где имя проекта идёт в заголовок отчёта (было: тихий `config_section`
    или голый `cfg.get`). `setup-onboarding.py` сознательно оставлен на мягком
    `config_section`-эквиваленте — это визард, который по своей природе
    работает ДО того, как проект существует (тот же принцип, что уже
    документирован в `config.py` для setup/onboarding-визардов).
  - **Легитимные границы, подтверждённые прогоном** (rc=0, но честное «данных
    нет», не притворство успехом): `structure-map.py` («Нет данных: запустите
    site-crawl…»), `redirect-map-audit.py` (`needs_input`), `obsidian-sync.py`
    («Загружено 0 сущностей») на пустом/несуществующем конфиге,
    `resolve-sources.py`, весь класс `*-health.py`/`*-fetch.py`/`*-audit.py`/
    `*-submit.py` health-чекеров и очередей (их вывод зависит от env/сети, а
    не от секции `project`, и честно показывает `needs_input`/`needs_credentials`
    при пустом конфиге).
  - **Класс `AttributeError` на `project: null` пройден до конца** (не только
    3 файла из отчёта): `access-key-assistant.py`, `launch-plan.py`,
    `obsidian-sync.py` (два места — f-string заголовка и `project_subfolder`,
    падало и на `project: null`, И на `project: "строка"`),
    `project-upgrade-assistant.py`, `setup-answer-plan.py`, `setup-blueprint.py`,
    `setup-control-plane.py`, `task-router.py`, `tool-stack-recommender.py` — 0
    трейсбеков на прогоне 99 файлов × 6 форм порчи (594 запуска).
  - **Интеграционные тесты** (`tests/test_t090_command_integration.py`, новый
    файл, 15 тестов): реально запускают мигрированный `scripts/*.py` как
    subprocess (не юнит-тест чистой функции) на `project: null`/пустом
    конфиге и требуют `returncode == 2` без трейсбека — включая
    `monthly-dashboard.py` и `db-sync.py`, чью решающую строку мутировал гейт.
    Проверено вручную: временный откат `require_section(...)` →
    `(cfg.get("project") or {})` в обоих файлах красит соответствующий тест
    (подтверждено и снова откачено).
  - **🟡4, формулировка приведена к факту.** Докстринг `config.py` и
    `_install_yaml_bypass_guard()` больше не говорят «структурно невозможно» —
    честно: рантайм-сторож защищает процессы, которые импортировали
    `seo_cycle_core.config` (верно для всех ~86 файлов, читающих конфиг);
    класс реально держит связка «рантайм-сторож + статический AST-тест в CI».
    AST-тест (`tests/test_no_yaml_bypass.py`) ужесточён: запрещает сам факт
    `import yaml`/`from yaml import ...` в любом `scripts/*.py`, кроме
    `config.py` (не только использование конкретных Loader-атрибутов —
    закрывает обход (а), новый файл без импорта ядра), детектирует
    `importlib.import_module(...)` с литералом `"yaml"` в аргументе (закрывает
    (в) в буквальной форме), и запрещает присваивание внутреннему флагу
    сторожа (`_guard_state`/бывший `_GUARD_ENABLED`) вне `config.py` (закрывает
    (б)). Публичный module-level `_GUARD_ENABLED = True` убран — сторож
    выключается только через `config._testing_disable_guard()`/
    `_testing_enable_guard()`, которые сами проверяют по стеку вызова, что
    зовущий файл лежит в `tests/`, и отказывают иначе. Три новых теста
    (`test_bypass_variant_a/b/c_...`) доказывают, что каждый вариант обхода
    из отчёта гейта теперь либо ловится статически (AST, вариант а и в), либо
    его ключ (`_guard_state`) больше нельзя присвоить снаружи без AST-находки
    (вариант б). Остаточный риск, названный вслух и не закрытый: обход (а) —
    файл, который вообще не импортирует `seo_cycle_core` — рантайм-сторож
    физически не может поймать (не установлен в этом процессе); закрывает
    только CI-гейт AST-теста. Динамическое построение имени модуля через
    конкатенацию строк (`"ya" + "ml"`) — тоже не покрыто литеральной
    AST-проверкой, признано остаточным риском, не покрывается сознательно.
  - Заодно найден и почищен class dead-code: ~25 файлов держали неиспользуемый
    `try: import yaml / except ImportError: ...` — остаток до-T-090
    миграции, само чтение уже шло через `seo_cycle_core.config`, но импорт
    оставался и не позволял ужесточить AST-тест. Удалён везде, где не было
    реального использования; ~10 файлов, которые реально писали YAML обратно
    (`yaml.safe_dump(...)` в setup/wizard-командах), переведены на новую
    `dump_yaml()` в `seo_cycle_core/config.py` (то же поведение, один вызов);
    8 файлов, проверявших `yaml is None`/`if yaml:` как признак «PyYAML
    установлен», переведены на новую `yaml_available()`.
  - Юнит-тестов было 861, стало 897 (+13 в `test_config_t090.py` — строгий
    `load_config` и доступ к `_testing_*_guard()`, +3 варианта обхода в
    `test_no_yaml_bypass.py`, +15 в новом `test_t090_command_integration.py`
    — интеграционные, subprocess-based) — все зелёные, включая мутационную
...
### Fix: install.sh — fetch тегов починен во ВСЕХ трёх местах, `--pin ... --sync` больше не принимает несуществующий тег (T-091, F-12/F-13/F-14)

Второй независимый враждебный QA (`optimize/reports/2026-09-07-seo-cycle-qa-v220.md`)
нашёл, что T-068 закрыл лишь одну из трёх точек, где `install.sh` тянет теги:
`update_store_only()` (только `--update`) получила `--force --prune --prune-tags` и
проверку кода возврата, а `install_or_update_repo()`/`install_or_update_optional_repo()`
— функции, через которые проходит **любой** обычный запуск установщика (первичная
установка, `--project` attach) — остались на конструкции с молчаливым `fetch --tags`,
которая и породила фантомный `v2.1.0`. F-12/F-13: обычный запуск оставлял переехавший
тег протухшим с кодом 0 и печатал неверную причину («offline?»). F-14: `--pin ... --sync`
принимал тег, которого на origin нет вовсе — установщик сам печатал эту форму как
рекомендуемую при обновлении, а `--sync` полностью отключает сверку с origin.

Закрыто по признаку, не по адресам:
- Все три места сведены к одной функции `fetch_tags_or_report()` (force+prune-tags,
  проверка кода возврата, честное сообщение о причине).
- **Круг 2 (2026-09-07 review, 🔴-1).** Первая версия закрыла только существующие
  вызовы поведенческими тестами — независимый гейт написал СВОЙ новый вызов
  `git fetch --tags`/`git ls-remote` в обход `fetch_tags_or_report()`, и он прошёл
  `bash -n`, `shellcheck -S error` и весь набор тестов молча: запрет был соглашением
  и комментарием, а не механизмом. Добавлен структурный (не поведенческий) гейт —
  `tests/test_installer_contracts.py::FetchChokePointStructuralGuardTest` — который
  читает `install.sh` целиком и утверждает: ровно одна исполняемая строка вида
  `git … fetch` во всём файле, и она внутри `fetch_tags_or_report()`; ровно три строки
  вида `git … ls-remote`, и все три — внутри `latest_tag()`/`ensure_worktree()`. Новый
  вызов ГДЕ УГОДНО ЕЩЁ меняет счётчик и роняет тест — доказано негативным контролем,
  который воспроизводит инъекцию ревьюера дословно (сначала на симуляции внутри теста,
  затем реальной правкой `install.sh` на диске с восстановлением после).
- Каждый успешный fetch/clone обновляет локальный манифест `<store>/.git/seo-cycle-origin-tags.tsv`
  (внутри `.git/`, никогда не в рабочем дереве и не в git-истории) — что реально видели
  на origin. `--sync` (принципиально без сетевых вызовов) теперь сверяет явный `--pin` с
  этим манифестом вместо того, чтобы слепо доверять локальному `git tag`: тег, которого
  манифест не подтверждал, отклоняется с кодом 1 и понятной причиной, лок не создаётся.
- Подсказка `install.sh --project ... --pin <тег> --sync`, которую установщик сам
  печатает как «Обновление», теперь включает `--update` первым шагом.
- Обычный (не `--sync`) запуск теперь САМОСТОЯТЕЛЬНО чинит расхождение локального тега
  с origin (force-fetch), а не оставляет его непроверенным до следующего `--update`.

**Честность сообщения (круг 2, 2026-09-07 review, 🟡-2).** `--pin <тег> --sync`
печатал просто «✓ sync завершён», хотя проверка по контракту офлайн (сверка с
локальным манифестом `seo-cycle-origin-tags.tsv`, не с origin) — если тег переехал
на origin ПОСЛЕ последнего `--update`, сообщение читалось как подтверждение с
сервера. Теперь оно называет дату манифеста явно: «офлайн-проверка по локальному
манифесту origin от YYYY-MM-DD HH:MM, не подтверждение с сервера — запусти
install.sh --update для реальной сверки».

**Осознанное изменение поведения (круг 2, 2026-09-07 review, 🟡-3): `install.sh
--upgrade-all --pin <тег>` на разошедшемся теге больше не отказывает.** Раньше — если
явный `--pin` указывал на тег, который успел переехать на origin, команда отказывала
кодом 1 и НЕ трогала лок ни одного проекта портфеля («ни один проект не перепинен —
портфель остался на прежних версиях»). Теперь `--upgrade-all` сначала вызывает
`ensure_store()`, который форсированным fetch'ем чинит расхождение локального тега с
origin (тот же fix, что и для обычного запуска выше) ДО того, как каждый проект
перепинивается — поэтому расхождения к моменту проверки уже нет, команда завершается
кодом 0, и в лок каждого живого проекта попадает подтверждённый коммит origin, а не
устаревший локальный. В логе это видно явной строкой `тег … переехал: старый → новый`
перед каждым `✓ lock: …`, так что смена коммита не тихая. Это строго честнее прежнего
поведения (в лок больше не может попасть дрейф), но это смена исхода команды, а не
косметика — называю явно, потому что именно этой командой T-055 перепинивает четыре
живых сайта.

Доказательства: 10 новых/адаптированных тестов в `tests/test_installer_contracts.py`
(45 тестов в файле вместо 35), включая мутационные проверки, которые снимают по одному
guard'у за раз и по паре guard'ов сразу — там, где T-091 создал защиту в глубину
(force-fetch чинит дрейф раньше, чем успевает сработать более старая проверка), тесты
это явно фиксируют, а не молчат об этом. Полный прогон: 852 теста (было 842 на теговом
коммите — +10 новых), `OK (skipped=1)`.

### Fix: тесты install.sh больше не трогают системный Python и без сети не виснут (F-18)

Прогон `python3 -m unittest discover -s tests` запускал настоящий `install.sh`, который
ставил `pyyaml requests pillow beautifulsoup4 google-auth` в СИСТЕМНЫЙ Python машины
(третий фолбэк — с `--break-system-packages`) и делал ~600 обращений к PyPI за один
прогон; без сети `pip` не имел таймаута и вис бесконечно вместо честного отказа.
`ensure_python_deps()` теперь уважает `SEO_CYCLE_SKIP_PYTHON_DEPS=1` (тестовый набор
всегда его выставляет) и передаёт `pip install` `--timeout 20`.

### Fix: журнал изменений противоречил сам себе разделом `[2.1.0]` (F-19)

Регрессия слияния T-067×T-068: раздел `## [2.1.0]` (публично не выходившая версия)
воскрес при мердже поверх переименования в `[2.2.0]`. Заголовок убран, содержимое
осталось частью `[2.2.0]` — раздела `[2.1.0]` в файле больше нет.

### Fix: настоящий UUID чужого блокнота NotebookLM в шаблоне проекта (F-20)

`config/project.template.yaml` — `notebooklm_url` указывал на конкретный чужой
блокнот; `INSTALL.md` велит копировать этот файл в каждый новый проект. Заменено на
`<notebook-id>`, как в тестовых фикстурах. Тем же заходом (2026-09-07 review, F-20-класс,
💭) убран такой же настоящий UUID блокнота из `scripts/vnext_audit_core.py:60`
(`EXPERT_SOURCES`) — не шаблон, но публикуемый скрипт с тем же чужим идентификатором.

### Fix: браузерные сборщики (WriterZen/GSC) тихо ставили npm-пакет в read-only vendor (T-091 round 2)

Независимый гейт T-091 (`optimize/reports/2026-09-07-review-T-091.md`) нашёл новый
инцидент того же класса, что F-18 (скрытая установка пакетов), но для npm: три
браузерных сборщика (`writerzen-browser-collect.py`, `gsc-indexing-export-browser.py`,
`gsc-request-indexing-browser.py`) при обычном запуске без флагов сами ставили
`playwright-core` через `npm install` в `~/.codex/vendor/seo-cycle-node` — каталог,
который README хранилища версий объявляет **только для чтения** (§3 глобальных
правил); установка была побочным эффектом обычного запуска, без явного согласия.

- Каталог по умолчанию перенесён в рабочее место инструмента, `~/.seo-cycle/vendor/seo-cycle-node`
  (тот же паттерн, что уже используют `~/.seo-cycle/env.global` и `~/.seo-cycle/reports`),
  и больше никогда не пишет в `~/.codex/vendor`.
- Флаг инвертирован: было `--skip-install-browser-runtime` (по умолчанию ставит,
  можно отказаться), стало `--install-browser-runtime` (по умолчанию НЕ ставит,
  нужно явное согласие) — отсутствующий playwright-core теперь сообщается
  (`"status": "missing"`), а не тихо устанавливается сетевым `npm install`.
- Доказано: `tests/test_browser_runtime_consent.py` — без флага ноль вызовов
  `subprocess.run` и ноль файлов на диске (мок), с флагом — ровно один вызов `npm
  install --prefix <sandbox>`; отдельно прогнан НАСТОЯЩИЙ (не мокнутый) `npm install`
  в песочнице с подменённым `HOME` — снапшот `find ~/.codex/vendor -maxdepth 2 | md5`
  до и после совпал побайтово.

### Fix: конфиг-инвариант — остаток по всем командам (T-093, Б2 QA-прогона v2.2.0)

Третий независимый враждебный прогон перед тегом (`optimize/reports/
2026-09-07-seo-cycle-qa-v220-final.md`, блокер Б2) нашёл, что 42 из 98
команд давали зелёный отчёт на `project: null`, 16 — на пустом конфиге.
T-067/T-090 закрывали этот класс дважды (4 и 3 круга) и оба раза матрица
строилась по **реестру команд**, а не по файлам, реально читающим конфиг в
обход реестра — часть оставалась открытой. T-093 строит матрицу по-другому:
grep по `scripts/*.py` на вызовы `load_config`/`load_yaml`/`require_config`/
`require_section`/`config_section`, а не по списку зарегистрированных
команд — итоговый круг 2 подтверждён **полным сканом всех 166
`scripts/*.py`** (`empty`/`project: null`, `--write`), не только
затронутых файлов.

**Реальный механизм — точечная проверка в каждом файле, не общий
загрузчик.** Первая версия положила проверку формы `project:` внутрь
`seo_cycle_core.config.load_config()` (один чек для всех вызывающих) — это
сломало бы уже протестированный T-090 контракт мягкой деградации нескольких
команд (`config_section()`, не `exit(2)`), поймано локальным прогоном и
реверчено ДО коммита в PR. `load_config()` в этом релизе **не изменён**.
Настоящий фикс — `require_section(cfg, "project", cfg_path)` (существующий
с T-090 хелпер) добавлен точечно в **50 файлов** (35 в круге 1 + 15 в круге
2 после независимого гейта, включая денежно-смежные, но конфиг-only правки
`spend-guard.py`/`usage-ledger.py`/`serpstat-audit.py`), плюс общий модуль
`seo_cycle_core/health.py` (`style="simple"`, закрывает 5 `*-health.py`
скриптов разом).

- **~49 скриптов**, читавших свой главный конфиг через нестрогий
  `load_yaml(cfg_path)` (тихо возвращает `{}` на пустом/только-комментарии
  файле), переведены на строгий `load_config(cfg_path)` — закрывает "файл
  есть, но пуст".
- `ai-bot-access-check.py` — импортировал нестрогий `load_yaml` через
  реэкспорт из `vnext_audit_core.py`; переведён на `load_config` тем же
  путём.
- `geo-citation-log.py` — раньше находил путь к конфигу, но НИКОГДА не
  разбирал его содержимое (`project_root_for()` берёт только расположение
  файла); T-092 закрыл разбор `{{{`/не-UTF-8 (`load_yaml`), T-093 закрыл
  оставшийся пустой/только-комментарии файл (`load_config`, возврат
  отброшен намеренно — скрипту не нужно ни одно поле).
- **Круг 2 (независимый гейт, R-2/R-3):** 8 подкоманд CLI из QA-отчёта
  (`seo-cycle context|report|kpi|spend|ledger|forecast|journey`) и
  `serpstat-audit.py` по-прежнему писали файлы на `project: null` — ни у
  одного из 12 задействованных скриптов не было теста, закрепляющего это
  как решение (значит это был баг, не контракт). Закрыты
  `require_section`: `automation-recommender.py`, `budget-mix-planner.py`,
  `client-report.py`, `context-pack.py`, `kpi-contract.py`,
  `position-progress.py`, `project-journey.py`, `redirect-map-audit.py`,
  `resolve-sources.py`, `seo-forecast.py`, `serpstat-audit.py`,
  `setup-gap-audit.py`, `spend-guard.py`, `token-waste-audit.py`,
  `usage-ledger.py`; плюс общий модуль `health.py` закрывает
  `gbp-health.py`, `google-ads-health.py`, `merchant-health.py`,
  `yandex-business-health.py`, `yandex-direct-health.py`.
  `geo-log` (`geo-citation-log.py`) остаётся законным исключением —
  никогда не читает `project.*` (см. ниже).
  **Каскад:** `seo-cycle status` делегирует финальную секцию
  `project-journey.py` — теперь наследует его отказ на `project: null`
  вместо тихого `rc=0`; пред-существующий тест T-067
  (`test_config_robustness.py::CliSectionAccessTest`) обновлён под новый,
  более строгий контракт (сам факт делегирования не изменился, изменилось
  только то, что делегат теперь отказывает).
- **Названы явно, не молча исключены** — 5 команд, чей основной режим
  законно работает без конфига вообще (проверено: `rc=0`, ноль файлов,
  сообщение называет реально отсутствующий ВХОД — не «Project: ?»):
  `metrika-cohorts.py`, `site-crawl.py`, `structure-map.py` (нужны
  `--live`/`--input-file`, не идентичность проекта), `spyfu-fetch.py usage`
  (учёт расхода на уровне аккаунта, не проекта), `research-package-repair.py`
  (работает по каталогу пакета, переданному аргументом, идентичность
  проекта не читает); плюс 2 setup-мастера, чья единственная работа —
  СОЗДАТЬ конфиг/интейк (`project-intake-wizard.py`, `setup-onboarding.py`,
  контракт T-090) и 3 файла, никогда не читающие `project.*`
  (`geo-citation-log.py`, `rag-query.py`; `redirect-map-audit.py` тоже не
  читает, но закрыт `require_section` в круге 2 всё равно — критерий
  приёмки безусловен по файлам, не только по содержательному вреду).
- **`dataforseo-fetch.py` исключён из проверки этим пакетом, а не тихо
  пропущен** — его `load_config(cfg_path)` (строка ~151) существует и
  достижим только через `armed_spend()`-гейт живого платного вызова;
  доказать проверку без ключей/сети (ограничение пакета) невозможно.
- Поведение на исправном конфиге не изменилось: побайтовое сравнение
  stdout/stderr/кода возврата/списка созданных файлов на 52 из 52
  затронутых скриптов (независимый гейт, круг 1) плюс дополнительная
  выборка круга 2 — различия только во временных метках/путях временных
  каталогов.
- Тесты: `tests/test_t093_config_null_remainder.py` — по одному тесту на
  (скрипт × форма плохого конфига), **458 тестов** (проверено командой:
  `python3 -c "import unittest; print(unittest.TestLoader().loadTestsFromName('tests.test_t093_config_null_remainder').countTestCases())"`
  → 458), каждый запускает реальный CLI-скрипт подпроцессом (не импорт
  хелпера) и проверяет и код возврата, и список созданных файлов.
  Мутации с якорем (снятие `require_section` без затрагивания
  синтаксиса — первая попытка на `ads-analytics.py` дала ложно-зелёный
  результат из-за нечаянной синтаксической ошибки, распознано и
  пересобрано ДО зачёта): 4 файла в круге 1 (`ads-analytics.py`,
  `xmlriver-health.py`, `writerzen-health.py`, `technical-mcp-health.py`) +
  2 файла в круге 2 (`client-report.py`, `usage-ledger.py`) — все мутанты
  убиты, восстановлены.
- Известный остаток (честно, не выдан за закрытый): `dataforseo-fetch.py`
  не верифицирован (см. выше); `project: {name: null}` (форма содержимого
  ВНУТРИ секции, не самой секции) по-прежнему даёт `rc=0` на нескольких
  аудитах — инвариант сформулирован про форму раздела, не его полей;
  граница в `monthly-runner.sh:93-108` (битый конфиг молча превращается в
  «работаем на умолчаниях», комментарий «NAMED boundary» в коде уже стоит)
  названа, не закрыта; `pulse.py` (оркестратор, не в исходном списке
  QA/R-2) пишет собственный scorecard-файл ДАЖE когда его под-шаги
  (db-sync/position-progress) отказывают на `project: null` — это
  корректно сигналит `rc=1` (не `rc=0`) и сам scorecard фиксирует именно
  отказ под-шагов, а не фиктивный успех, но формально нарушает букву
  критерия «ноль файлов»; не тронут в этом пакете, назван явно.

### Fix: прокси-обход денежного шлюза закрыт; клиентский отчёт отказывает на битом имени проекта; граница защиты приведена к факту (T-094, F-2/F-3/F-4 четвёртого прогона)

Четвёртый независимый враждебный QA (`optimize/reports/2026-09-07-seo-cycle-qa-v220-final2.md`)
нашёл три жёлтых находки одного класса: **код лучше, чем текст о нём** — граница
защиты не поспевала за тем, что механизм реально делает или не делает.

- **F-2 — денежный шлюз закрыт на HTTPS-пути через прокси (порт, точка на конце и их сочетание — круг 3 ниже); plain-`http://` через прокси остаётся открытым путём, назван честно.** При заданной
  `HTTPS_PROXY`/`HTTP_PROXY` `socket.getaddrinfo`/`socket.socket.connect`
  (уровень, на котором T-089 круг 3 поставил гейт) видят только хост **прокси**
  — платное имя (`api.dataforseo.com` и т.п.) уезжает внутрь HTTP `CONNECT`-
  заголовка, который строит `http.client.HTTPConnection.set_tunnel(host, ...)`.
  Это не гипотетический путь: `config/region-profiles/ru.yaml:11,47`
  **рекомендует** ходить в DataForSEO через прокси для основного региона
  использования. Патч добавлен туда же, где уже проверяется резолвинг имени:
  `spend_guard._guarded_set_tunnel` оборачивает `http.client.HTTPConnection.
  set_tunnel` и вызывает тот же `_check_host()` до отправки CONNECT — точка
  единственная, потому что и `urllib.request` (`AbstractHTTPHandler.do_open`
  вызывает `h.set_tunnel(req._tunnel_host, ...)` напрямую), и `requests`/
  `urllib3` (`HTTPSConnectionPool._prepare_proxy` вызывает `conn.set_tunnel(...)`,
  чья реализация заканчивается `super().set_tunnel(host, ...)` — то есть тем же
  патченным именем на базовом классе) заканчиваются в этой одной функции.
  Проверено без единого живого запроса (тот же приём, что у QA — шим,
  несуществующий локальный порт прокси):
  `HTTPS_PROXY=http://127.0.0.1:9 python3 -c "urllib.request.urlopen('https://api.dataforseo.com')"`
  и то же через `requests.get(...)` — оба теперь падают `SpendNotArmedError`
  ДО обращения к прокси, а не проходят молча. Армированный вызов
  (`with armed_spend(...)`) по-прежнему доходит до сетевого уровня без
  постороннего вмешательства гейта (позитивный контроль — падает уже на
  `URLError`, не на гейте). Тесты: `tests/test_t094_proxy_tunnel_gate.py`
  (исходно 5 тестов: патч установлен, неармированный платный хост через
  туннель отказывает, свободный хост не трогается, армированный хост
  проходит, регистр/точка на конце не обходят проверку) — мутация (снятие
  `_check_host()` из `_guarded_set_tunnel`) убивает тест `unarmed_paid_host_
  via_tunnel_is_refused`, восстановлено. `gate_installed()` и докстринг
  `spend_guard.py` обновлены: патч `set_tunnel` — шестой патченный вызов.

  **Круг 2 (независимый гейт, `optimize/reports/2026-09-07-review-T-094.md`,
  🔴 R-1): первая версия заявляла «граница больше не молчит про прокси — он
  закрыт», и это было ложным заявлением о полноте.** Ревьюер поднял
  собственный локальный TCP-слушатель (наружу — ни байта) и живьём доказал
  два открытых пути платным именем на проводе:
  1. **URL с явным портом через `HTTPS_PROXY`** (`https://api.dataforseo.com:443/...`)
     — `req._tunnel_host` приходит в `_guarded_set_tunnel` С портом,
     `_normalize_host` его не отрезала, сравнение с `PAID_HOSTS` не
     срабатывало. Закрыто: `_normalize_host` теперь зовёт новую
     `_strip_port()` (IPv6-осторожную — `[::1]:443` → `::1`, голый `::1`
     не трогается, режется только форма `host:port` с ровно одним `:`).
     Тесты `test_explicit_port_does_not_evade_the_tunnel_check` +
     `test_strip_port_handles_bracketed_ipv6_and_leaves_bare_ipv6_alone`;
     мутация (откат `_strip_port` в `_normalize_host`) убивает первый,
     восстановлено. Повторная репродукция сценария ревьюера (порт в URL,
     тот же офлайн-приём — несуществующий локальный порт прокси) теперь
     даёт `SpendNotArmedError`.
  2. **Plain-`http://`-цель через прокси — НЕ закрыт, назван честно, не
     заявлен закрытым.** `urllib`/`requests` для http-цели через прокси не
     строят CONNECT-туннель вовсе — запрос уходит абсолютным URI прямо в
     прокси (`GET http://api.dataforseo.com/... HTTP/1.1`), ни
     `set_tunnel`, ни резолвинг платного имени в этом процессе не
     происходят. Закрытие потребовало бы перехвата на уровне построения
     запроса (`http.client.HTTPConnection.putrequest`/переписывания URI) —
     другая, более крупная правка, не одна строка; не сделана в этом
     пакете, названа в докстринге `spend_guard.py` («Does NOT buy») прямым
     текстом вместо того, чтобы молчаливо считаться закрытой фиксом выше.
     Достижимость из кода этого репозитория низкая: все 7 платных клиентов
     (`dataforseo-fetch.py`, `spyfu-fetch.py`, `serpstat-fetch.py`,
     `serpstat-audit.py`, `keyso-save.py`, `yandex-direct-fetch.py`,
     `atp-fetch.py`) захардкожены на `https://` без порта, прокси из
     конфига нигде не читают (`grep -rn "proxies=\|ProxyHandler\|set_proxy"
     scripts/` минус сам `spend_guard.py` — пусто); риск — для рукописного/
     будущего скрипта с платным `http://`-хостом, который
     `tests/test_t089_closed_world_hosts.py` поймает как неклассифицированный
     литерал в момент написания.

  **Круг 3 (независимый гейт, `optimize/reports/2026-09-07-review-T-094-round2.md`,
  🔴 R2-1): порядок двух операций внутри `_normalize_host` пропускал
  комбинированную форму.** Отсечение точки на конце (`.rstrip(".")`)
  стояло ДО отсечения порта (`_strip_port`) — `.rstrip(".")` режет только
  с самого конца строки, поэтому `api.dataforseo.com.:443` превращался в
  `api.dataforseo.com.` (порт ещё цел, отрезалась не та точка), сравнение с
  `PAID_HOSTS` не срабатывало. Ревьюер провёл это живьём против локального
  слушателя (наружу — ни байта) — доказано платным именем на туннеле.
  Заголовок этого пункта до круга 3 звучал «закрыт на HTTPS-пути через
  прокси, с портом и без» — это снова было заявлением шире факта. Закрыто:
  порядок операций в `_normalize_host` поменян — сначала `_strip_port`,
  потом `.rstrip(".")`. Тест `test_trailing_dot_and_port_combined_do_not_
  evade_normalize`; мутация (откат порядка операций) убивает его,
  восстановлено. Круг 3 подтверждён ревьюером на его собственном стенде до
  публикации фикса (все проверенные формы прокси — оба регистра
  переменных, прокси с учётными данными, исключения, прокси из кода —
  отвергаются; армированный путь не сломан; сужения нет — свободный адрес
  с портом и локальные адреса не трогаются).
- **F-3 — `client-report.py` отказывает вместо отчёта с именем каталога
  вместо клиента.** `project.name: null` (раздел `project` — непустой
  мэппинг, `require_section()` его пропускал) раньше давал `EXIT=0` и писал
  4 файла, подставляя `project_root.name` в шапку документа, который уходит
  человеку. Добавлена точечная проверка `config_section(cfg, "project").
  get("name")` сразу после `require_section()` — пусто/`null` → `ERROR` в
  stderr, код 2, ни одного файла. Тест: `tests/test_t094_client_report_null_
  name.py` (отказ на `project.name: null` с нулём файлов + позитивный
  контроль — обычное имя по-прежнему пишет отчёт). Мутация (нейтрализация
  проверки) убивает отрицательный тест, восстановлено. Повтор репродукции
  QA-отчёта (`mkdir cr2; printf 'project:\n  name: null\n' > c.yaml;
  client-report.py c.yaml --write`) теперь даёт `EXIT=2`, `find seo -type f`
  → пусто. **Известный остаток, не тронут в этом пакете (независимый гейт,
  💭 R-2):** `project.name` из одних пробелов (`"   "`) проходит проверку
  (`not "   "` — ложно в Python) и пишет 4 файла с шапкой `# Отчёт по
  продвижению —    ` (пустое место, а не подставленное чужое имя каталога —
  вред другого рода, но документ недоделанный уходит как есть). Починка —
  `.strip()` в условии на `client-report.py:292` (по номеру строки на
  коммите ревью; в этой ветке — то же условие, добавленное этим пакетом) —
  оставлена как отдельная находка после тега, не смешана с этим циклом.
- **F-4 — исключение мастеров настройки: текст приведён к факту, перечень —
  своей командой, не на веру.** Обоснование (`setup-onboarding.py`/
  `project-intake-wizard.py` обязаны работать без готового конфига — T-090,
  F-8) остаётся законным и в этом пакете не тронуто. Круг 1 этого пакета
  принял число QA «12, не 3» без перечня — независимый гейт (`optimize/
  reports/2026-09-07-review-T-094.md`, R-3) вернул это: критерий пакета
  требует именно перечень, подтверждённый командой, не число со ссылкой на
  чужой отчёт. Круг 2 — свой прогон, без единого живого запроса (все
  исходящие подключения направлены на `127.0.0.1:1`, локально отклоняющий
  порт — платные и свободные хосты одинаково не покидают машину): каждый из
  100 скриптов, не входящих в `PROJECT_REQUIRED_SCRIPTS`/
  `NO_PROJECT_NEEDED_SCRIPTS` (`tests/test_t093_config_null_remainder.py`),
  запущен в чистом временном каталоге с `seo-cycle.yaml` = `just a string`
  (та же форма, что даёт `Project: ? (?)` у обоих мастеров) — **одна** из
  семи форм QA-матрицы, не все семь (полный пересчёт 1113 ячеек по всем
  формам этим пакетом не сделан — см. ниже). Результат: **8 скриптов дали
  `EXIT=0`** на этой форме —
  `project-intake-wizard.py` и `setup-onboarding.py` (фиктивная личность
  проекта — уже названный F-4 случай, не тронуты по обоснованию выше),
  `secret-scan.py` и `skill-manifest-validate.py` (не читают `project.*` —
  секрет-скан и валидатор манифеста работают по своим данным), `serp-intel.py`
  (данные из `seo/research-package/*.json`, `project.*` не читает),
  `research_package_repair_core.py` и `vnext_audit_core.py` (библиотечные
  модули без `if __name__ == "__main__":` — запуск как скрипт не выполняет
  ничего, `EXIT=0` тривиален), `seo_cycle_cli.py` без подкоманды (печатает
  `usage`/список команд и завершается кодом 0 — штатное поведение
  `argparse` без обязательного позиционного аргумента, не касается
  `project.*` вовсе). (Круг 3, независимый гейт `optimize/reports/
  2026-09-07-review-T-094-round2.md`, R2-3: `triggers-eval.py` в этом
  списке был ошибкой круга 2 — его `EXIT=0` в том прогоне объясняется тем,
  что скрипту не передавался `--project-yaml`, то есть он вообще не читал
  битый `seo-cycle.yaml`, а НЕ отказывал на нём; с `--project-yaml`,
  указывающим на ту же битую форму, `triggers-eval.py` даёт `EXIT=2` —
  отказывает корректно, в списке «зелёных» ему не место.)
  Расхождение с числом QA (12 против 8 здесь) объясняется методикой: QA
  прогнала 7 форм битого конфига и полный список из 159 скриптов, этот
  круг — 1 форму и 100 (из тех, что вообще способны прочитать
  `seo-cycle.yaml`). Полное совпадение чисел не заявляется — заявляется
  только то, что реально прогнано и проверено выводом команды выше (скрипт
  прогона и полный вывод — `optimize/.agents/runtime/tasks/T-094-...md`,
  «Результат»). Из восьми — ДВА (`setup-onboarding.py`, `project-intake-
  wizard.py`) пишут документ с подставной личностью проекта (это и есть
  находка F-4, обоснование её не отменяет), остальные шесть не читают
  `project.*` вовсе и «Project: ? (?)»-класса не воспроизводят.
- **Отдельно проверено (item 4 задачи — все три раздела «Граница защиты»
  сверены с фактом):**
  - Деньги (докстринг `spend_guard.py` + этот файл, `CHANGELOG.md:60-67`) —
    F-2 закрыт для HTTPS через прокси (с портом и без — круг 2 добавил отрезание
    порта, R-1). Круг 2 независимого гейта (`optimize/reports/
    2026-09-07-review-T-094.md`) нашёл, что первая версия заявляла «закрыт»
    не по факту: plain-`http://`-цель через прокси НЕ строит CONNECT-туннель
    вовсе и остаётся открытым путём — назван прямо в докстринге
    `spend_guard.py` (раздел «Does NOT buy»), не заявлен закрытым нигде.
    Достижимость из кода этого репозитория низкая (все 7 платных клиентов —
    `https://` без порта, прокси из конфига не читают), назван как
    структурный, не как проектный, риск.
  - Конфиг (абзац «Известный остаток», `CHANGELOG.md:1541-1553`) — был
    занижен (не называл `client-report.py`/мастеров), F-3 закрыт кодом
    (круг 2: остаток на имени из одних пробелов не тронут, назван ниже),
    F-4 приведён к факту текстом и своей командой выше.
  - Установщик — отдельного раздела «Граница защиты» для `install.sh` в
    файле по-прежнему нет (ближайшее — разбор починок T-091 выше, не
    заявление о границе). Для симметрии с денежным разделом: `install.sh`
    НЕ защищает от (1) поддельного/скомпрометированного origin-репозитория
    (доверие к `github.com/turvodnik/seo-cycle` не проверяется криптографически,
    только SHA/тег сверкой с самим origin); (2) параллельного другого
    процесса, пишущего в тот же `$SHARED_DIR`/лок-файл проекта одновременно
    (нет файловой блокировки); (3) утраты/повреждения локального манифеста
    `seo-cycle-origin-tags.tsv` между обновлениями — `--sync` тогда либо
    откажет консервативно, либо (если манифеста нет вовсе) не имеет базы
    для офлайн-сверки. Ни один из трёх не был найден QA как живой баг в этом
    прогоне — добавлены как честное дополнение к разделу T-091, не как
    новая находка.

Полный прогон (сверка числа тестов в сборке с локальным по логам заданий) и
хеши коммитов — в «Результате» пакета T-094 (`optimize/.agents/runtime/tasks/
T-094-boundary-text-and-client-report.md`), не дублируются здесь.
