...
- **F-2 — денежный шлюз закрыт на HTTPS-пути через прокси (с портом и без); plain-`http://` через прокси остаётся открытым путём, назван честно.** При заданной
...
- **F-4 — исключение мастеров настройки: текст приведён к факту, перечень —
  ...
  ничего, `EXIT=0` тривиален), `triggers-eval.py` (не читает `seo-cycle.yaml`
  вовсе без явного `--project-yaml`, что этот прогон не передавал — тот же
  класс, что и `geo-citation-log.py`/`rag-query.py`, не относится к
  «Project: ? (?)»-классу; отдельно проверено этим пакетом с
  `--project-yaml` на ту же битую форму — `EXIT=2`, отказывает корректно).
