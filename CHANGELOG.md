...
валидного ввода результат побайтно тот же, что в UTF-8-локали).
...
терминал; нет ни того, ни другого — внятная строка в stderr и код 3 без записи
файлов (на `--defaults` молча не переключается: человек просил опрос); `EOF`
и Ctrl-C — тот же честный исход. `init-project.sh` при ненулевом коде мастера
