...
- `--sync` больше не тянет `ensure_store` (без сети и pip) (F-7).
- `scripts/install-schedule.sh`: флаг `--scope <имя>` оборачивает job в
  `ai-secret run <scope> -- /usr/bin/env PATH=...`; без него — предупреждение в stderr
  вместо тихой деградации fetch; добавлен `--dry-run` (I-061).
- Тесты: `tests/test_installer_contracts.py` (7 кейсов), `tests/test_install_schedule.py`
  (3 кейса) — на временных каталогах и фиктивном origin, без сети и без живых проектов.
