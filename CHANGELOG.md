...
- `README.md`: убран жёсткий номер версии из шапки (текущая версия — файл `VERSION`,
  дублировать в тексте не будем — быстро расходится); TL;DR переписан на канонический
  путь установки (`install.sh` store+attach), curl-bootstrap оставлен как «быстрый
  старт для нового проекта» с явной пометкой, что общий код живёт в хранилище; убраны
  остаточные упоминания `.env` со значениями (WordPress MCP, шаринг).
- `INSTALL.md` Шаг 4 переписан под §5 глобальных правил: регистрация ключей через
  `ai-secret set`, потребление через `ai-secret run <scope> -- seo-cycle …`, перенос
  legacy `.env` через `ai-secret import`. Убраны все примеры со значениями, похожими на
  настоящие (`atp_pk_live_...`, `xxxx xxxx xxxx xxxx`).
