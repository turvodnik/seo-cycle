...
- После first-run или изменения policy запускай `setup-gap-audit.py --write` и закрывай missing fields через `seo/setup/setup-questionnaire.csv` / `seo/setup/setup-gap-audit.md` до широких циклов, платных API/LLM, массового local/ecommerce или automations.
...
- Перед чтением подробных setup-отчётов открывай `context-pack.py --write` / `seo/setup/context-pack.md`, затем `setup-gap-audit.py --write` / `seo/setup/setup-questionnaire.csv` и `seo/setup/setup-gap-audit.md`, затем `launch-plan.py --write` / `seo/setup/launch-plan.md`; это low-token вход, заполняемый worksheet и первый экран проекта.
...
- After first run or policy changes, run `setup-gap-audit.py --write` and close missing fields through `seo/setup/setup-questionnaire.csv` / `seo/setup/setup-gap-audit.md` before broad cycles, paid API/LLM use, bulk local/ecommerce work, or automations.
...
- Before reading detailed setup reports, run `context-pack.py --write` / open `seo/setup/context-pack.md`, then `setup-gap-audit.py --write` / open `seo/setup/setup-questionnaire.csv` and `seo/setup/setup-gap-audit.md`, then `launch-plan.py --write` / open `seo/setup/launch-plan.md`; these are the low-token entry point, fillable worksheet, and first project screen.
