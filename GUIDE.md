...
| **Token/provider evidence** | `token-waste-audit.py`, `perplexity-health.py`, `notebooklm-health.py`, `perplexity-collect.py`, `notebooklm-source-pack.py`: проверка raw/large артефактов, provider health и запись raw/distillate/vector source packs без хранения паролей. |
...
- `seo/research-package/semantic-core.cleaned.csv`, `seo/research-package/semantic-core.rejected.csv`, `seo/research-package/semantic-core.resynced.csv`, `seo/research-package/entity_coverage.jsonl`, `seo/research-package/content-plan.orphan-backlog.csv`, `seo/research-package/serp-validation-plan.csv`, `seo/research-package/spoke-opportunities.csv`, `seo/research-package/entity-graph-quality.md/json` — repair layer для findings сравнительного отчёта: очистка ядра, ресинхрон URL/cluster IDs, sync entity map, агрегация Google NLP, orphan backlog, план SERP-проверки, phase-2 spokes и качество entity graph.
...
- `seo/research/raw/*`, `seo/research/distillates/*`, `seo/research/vector/source_pack.jsonl` — source evidence contract для Perplexity/NotebookLM: raw хранится на диске, downstream context получает только distillates/latest-summary + citations.
...
- Для writing-фазы после research package запускай `research-package-quality.py --write`, затем repair layer по findings (`semantic-core-clean.py`, `semantic-core-resync.py`, `entity-map-sync.py`, `google-nlp-aggregate.py`, `orphan-url-resolver.py`, `serp-validation-plan.py`, `spoke-opportunity-audit.py`, `entity-graph-quality.py`), затем `page-outline-v2.py --all-mvp --write` или `--priority P1 --write`, затем `page-outline-quality.py --write`. Для черновика читай `copywriting_playbook` и `writer_prompt_packet`, после черновика запускай `draft-quality-gate.py`; raw research не загружай, пока gate не просит конкретный источник.
...
| `orphan-url-resolver.py` / `serp-validation-plan.py` / `spoke-opportunity-audit.py` | Repair архитектуры: закрывает orphan internal URLs backlog-строками, планирует недостающие SERP проверки для page-type решений и выносит GSC long-tail в phase-2 spokes | `python3 orphan-url-resolver.py ./research-package --write`; `python3 serp-validation-plan.py ./research-package --write`; `python3 spoke-opportunity-audit.py ./research-package --write` | `content-plan.orphan-backlog.csv`, `serp-validation-plan.csv`, `spoke-opportunities.csv`, отчёты `.md/.json` |
| `page-outline-v2.py` | Генерирует глубокие H2/H3 page briefs из research package: word counts, H3 allocation, intro/conclusion, SEO meta, Key Takeaways, FAQ, visual plan, section bridges, writer handoff, `copywriting_playbook`, `writer_prompt_packet`, copywriting details, source slots, acceptance criteria, entities, keywords, visuals, copywriter notes, Answer Units, evidence, schema, internal links, synthetic prompts, E-E-A-T guard | `python3 page-outline-v2.py ./research-package --all-mvp --write`; одиночно: `--page "/tools/virtual-hair-color-try-on/"` | `page-outlines-v2/<page>.md/json` |
| `page-outline-quality.py` | Quality gate для page briefs: word-count drift, H3/H2 mismatch, SERP/page-type lock, intro/conclusion, SEO meta, Key Takeaways, FAQ, handoff, copywriting playbook, writer prompt packet, revision checklist, fact-check queue, schema, internal links, Answer Units, source slots, acceptance criteria, entity orphans, bridges, visuals, trust limits, synthetic prompts, fabricated first-person expertise; даёт 10-критериальный scorecard и action plan | `python3 page-outline-quality.py ./research-package --write`; коротко: `--format markdown` | `page-outline-quality.md/json`, `latest-page-outline-quality.md/json`; exit 1 при critical findings |
| `entity-graph-quality.py` / `draft-quality-gate.py` | Quality gates после repair/briefing: ловят дубли/сироты триплетов, entity weights без source, а в черновике — missing H2/H3, обязательные ссылки, source/proof slots и unsafe first-person expertise | `python3 entity-graph-quality.py ./research-package --write`; `python3 draft-quality-gate.py draft.md --outline page-outlines-v2/page.json --write` | `entity-graph-quality.md/json`, `<draft>.draft-quality-gate.md/json` |
...
| **Token/provider evidence** | `token-waste-audit.py`, `perplexity-health.py`, `notebooklm-health.py`, `perplexity-collect.py`, `notebooklm-source-pack.py`: raw/large artifact checks, provider health, and raw/distillate/vector source packs without password storage. |
...
- `seo/research-package/semantic-core.cleaned.csv`, `seo/research-package/semantic-core.rejected.csv`, `seo/research-package/semantic-core.resynced.csv`, `seo/research-package/entity_coverage.jsonl`, `seo/research-package/content-plan.orphan-backlog.csv`, `seo/research-package/serp-validation-plan.csv`, `seo/research-package/spoke-opportunities.csv`, `seo/research-package/entity-graph-quality.md/json` — repair layer for comparison-report findings: semantic-core cleanup, URL/cluster ID resync, entity-map sync, Google NLP aggregation, orphan backlog, missing SERP validation plan, phase-2 spokes, and entity graph quality.
...
- `seo/setup/token-waste-audit.md`, `seo/setup/perplexity-health.md`, `seo/setup/notebooklm-health.md` — low-token/provider readiness: raw/large artifact findings, Perplexity persistent app/browser/API optional mode, NotebookLM MCP/export fallback.
- `seo/research/raw/*`, `seo/research/distillates/*`, `seo/research/vector/source_pack.jsonl` — source evidence contract for Perplexity/NotebookLM: raw stays on disk; downstream context gets only distillates/latest-summary + citations.
...
- For the writing phase after a research package, run `research-package-quality.py --write`, then the repair layer for findings (`semantic-core-clean.py`, `semantic-core-resync.py`, `entity-map-sync.py`, `google-nlp-aggregate.py`, `orphan-url-resolver.py`, `serp-validation-plan.py`, `spoke-opportunity-audit.py`, `entity-graph-quality.py`), then `page-outline-v2.py --all-mvp --write` or `--priority P1 --write`, then `page-outline-quality.py --write`. Draft from `copywriting_playbook` and `writer_prompt_packet`, then run `draft-quality-gate.py`; do not load raw research unless the gate requests a specific source.
...
| `orphan-url-resolver.py` / `serp-validation-plan.py` / `spoke-opportunity-audit.py` | Architecture repair: turns orphan internal URLs into backlog rows, plans missing SERP checks for page-type decisions, and promotes measured GSC long-tail into phase-2 spokes | `python3 orphan-url-resolver.py ./research-package --write`; `python3 serp-validation-plan.py ./research-package --write`; `python3 spoke-opportunity-audit.py ./research-package --write` | `content-plan.orphan-backlog.csv`, `serp-validation-plan.csv`, `spoke-opportunities.csv`, `.md/.json` reports |
| `page-outline-v2.py` | Generates deep H2/H3 page briefs from a research package: word counts, H3 allocation, intro/conclusion, SEO meta, Key Takeaways, FAQ, visual plan, section bridges, writer handoff, `copywriting_playbook`, `writer_prompt_packet`, copywriting details, source slots, acceptance criteria, entities, keywords, visuals, copywriter notes, Answer Units, evidence, schema, internal links, synthetic prompts, E-E-A-T guard | `python3 page-outline-v2.py ./research-package --all-mvp --write`; single page: `--page "/tools/virtual-hair-color-try-on/"` | `page-outlines-v2/<page>.md/json` |
| `page-outline-quality.py` | Quality gate for page briefs: word-count drift, H3/H2 mismatch, SERP/page-type lock, intro/conclusion, SEO meta, Key Takeaways, FAQ, handoff, copywriting playbook, writer prompt packet, revision checklist, fact-check queue, schema, internal links, Answer Units, source slots, acceptance criteria, entity orphans, bridges, visuals, trust limits, synthetic prompts, fabricated first-person expertise; returns a 10-criterion scorecard and action plan | `python3 page-outline-quality.py ./research-package --write`; short mode: `--format markdown` | `page-outline-quality.md/json`, `latest-page-outline-quality.md/json`; exits 1 on critical findings |
