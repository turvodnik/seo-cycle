...
- собирает семантику из 10+ источников (Яндекс, Google, XMLRiver, Serpstat, SpyFu, NeuronWriter, LLM-CLI, AnswerThePublic, Perplexity);
...
| **Token/provider evidence** | `token-waste-audit.py`, `perplexity-health.py`, `notebooklm-health.py`, `xmlriver-health.py`, `perplexity-collect.py`, `notebooklm-source-pack.py`, `xmlriver-source-pack.py`: проверка raw/large артефактов, provider health и запись raw/distillate/vector source packs без хранения паролей. |
...
- `seo/setup/token-waste-audit.md`, `seo/setup/perplexity-health.md`, `seo/setup/notebooklm-health.md`, `seo/setup/xmlriver-health.md` — low-token/provider readiness: raw/large artifact findings, Perplexity persistent app/browser/API optional mode, NotebookLM MCP/export fallback, XMLRiver readiness/prices/capabilities.
...
| `page-outline-v2.py` | Генерирует глубокие H2/H3 page briefs из research package: word counts, H3 allocation, `metrics_rollup` (volume/clicks/impressions/priority/top supporting keywords), intro/conclusion, SEO meta, Key Takeaways, FAQ, visual plan, section bridges, writer handoff, `copywriting_playbook`, `writer_prompt_packet`, copywriting details, source slots, acceptance criteria, entities, keywords, visuals, copywriter notes, Answer Units, evidence, schema, internal links, synthetic prompts, E-E-A-T guard | `python3 page-outline-v2.py ./research-package --all-mvp --write`; одиночно: `--page "/tools/virtual-hair-color-try-on/"`; архив дублей: `--archive-legacy-briefs` | `page-outlines-v2/<page>.md/json`; optional `archive/legacy-briefs/` |
...
- collects keywords from 10+ sources (Yandex, Google, XMLRiver, Serpstat, SpyFu, NeuronWriter, LLM-CLI, AnswerThePublic, Perplexity);
...
| **Token/provider evidence** | `token-waste-audit.py`, `perplexity-health.py`, `notebooklm-health.py`, `xmlriver-health.py`, `perplexity-collect.py`, `notebooklm-source-pack.py`, `xmlriver-source-pack.py`: raw/large artifact checks, provider health, and raw/distillate/vector source packs without password storage. |
...
| `page-outline-v2.py` | Generates deep H2/H3 page briefs from a research package: word counts, H3 allocation, `metrics_rollup` (volume/clicks/impressions/priority/top supporting keywords), intro/conclusion, SEO meta, Key Takeaways, FAQ, visual plan, section bridges, writer handoff, `copywriting_playbook`, `writer_prompt_packet`, copywriting details, source slots, acceptance criteria, entities, keywords, visuals, copywriter notes, Answer Units, evidence, schema, internal links, synthetic prompts, E-E-A-T guard | `python3 page-outline-v2.py ./research-package --all-mvp --write`; single page: `--page "/tools/virtual-hair-color-try-on/"`; duplicate legacy archive: `--archive-legacy-briefs` | `page-outlines-v2/<page>.md/json`; optional `archive/legacy-briefs/` |
